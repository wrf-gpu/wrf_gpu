...
<<<CRITIQUE: Consider changing "WRF v4 Port" to "WRF-compatible replay prototype" or similar. The body repeatedly says this is not a line-by-line WRF port and not a standalone operational replacement; the title currently invites reviewers to judge it as a completed WRF v4 port.>>>
...
<<<CRITIQUE: This is a policy-sensitive authorship choice. If the target is arXiv only, explicitly verify arXiv's current policy and add a disclosure note. If the target includes journals or proceedings, move AI systems from author line to "tools/contributions" unless the venue explicitly permits AI co-authors.>>>
...
<<<CRITIQUE: The abstract currently headlines the pre-fix 324.78 s / 50.20x result even though Section 8.2 says that path was running incorrect physics and the corrected-physics path is 708.32 s / 23.02x. A reviewer will call this cherry-picking unless the abstract leads with the current corrected-physics number and treats 324.78 s as historical/diagnostic. Also replace "peak process memory" with "nvidia-smi memory.used after warm probe" unless the proof object actually records peak allocation.>>>
...
<<<CRITIQUE: This contribution is stale after the algorithmic fix. If the current scientific claim uses correct physics, the contribution should report 708.32 s and 23.02x, with 324.78 s / 50.20x presented only as the pre-fix benchmark that exposed the workflow's overclaim risk.>>>
...
<<<CRITIQUE: The citation is a generic AceCAST documentation/manual entry, not a stable paper or exact page for the 5-14x claim. Either cite the exact source named in the research brief, quote it conservatively, or soften to "reported marketing/technical materials claim".>>>
...
<<<CRITIQUE: Add the brief's COSMO/CH model and more WRF-specific GPU-port context here, or reviewers may see this as comparing against adjacent global/climate systems while skipping the most relevant regional/WRF acceleration history. Also verify the 2026 ICON bibliographic metadata before submission.>>>
...
<<<CRITIQUE: Do not leave TODO_Mollick_AI_authorship in a draft sent outside the repo. Either add a verified BibTeX entry from a real source or remove the sentence. Also replace secondary arXiv-policy news citations with official arXiv/publisher policy pages if possible.>>>
...
<<<CRITIQUE: This reads like an authorship argument rather than a methods result. Move most of it to Author Contributions/Disclosure and make the Methods section focus on reproducible process mechanics: contract fields, branch policy, review gates, proof-object schemas, and rejection examples.>>>
...
<<<CRITIQUE: This is the paper's strongest methodological contribution, but it is still abstract. Add a small table or boxed example with one real sprint contract, its proof objects, commands, and closeout verdict. That will make the method auditable rather than anecdotal.>>>
...
<<<CRITIQUE: Add the post-fix proof objects here too (`post_fix_speedup.json`, `post_fix_skill_diff.json`, `invariant_preservation.json`). Otherwise the proof-object list lags the current narrative.>>>
...
<<<CRITIQUE: Good detail, but add the exact proof object or table reference for these grid/timestep values. A reviewer should not have to infer that they come from `wall_clock.json` and `reproducibility_v2.json`.>>>
...
<<<CRITIQUE: Cite the exact M6 closeout/invariant proof path here. Also explain what "bitwise" covers: which fields, what step boundary, validation mode or operational mode, and whether physics/boundaries were active.>>>
...
<<<CRITIQUE: This is now chronologically ambiguous. Section 8.2 says the combined fix changed the cadence to 180 and re-ran the 24 h pipeline. Rewrite this paragraph as "In the pre-fix path..." and then add the post-fix status and remaining radiation/land-state limitations.>>>
...
<<<CRITIQUE: `step_feasibility.json` records `nvidia-smi` memory.used after warm, not necessarily peak process allocation. Rephrase to avoid overstating memory evidence; keep the "one-step feasibility probe" caveat.>>>
...
<<<CRITIQUE: This should move from caveat to required reproducibility artifact: include exact Python, jax, jaxlib, CUDA, driver, NVIDIA toolkit, XLA flags, git commit, and proof-object commit hashes before submission.>>>
...
<<<CRITIQUE: Rename this table. It mixes pre-fix incorrect-physics measurements with post-fix corrected-physics measurements. Suggested headings: "Pre-fix diagnostic path" and "Current corrected-physics path".>>>
...
<<<CRITIQUE: "same" is acceptable in prose but weak in a proof-object table. Repeat the exact proof path or use a table note.>>>
...
<<<CRITIQUE: These should be the headline performance values if the paper claims "with correct physics". The abstract, contributions, and discussion still foreground the faster pre-fix path.>>>
...
<<<CRITIQUE: "Lowest defensible headline ratio" is no longer true after the post-fix run; the lowest defensible current corrected-physics ratio is 23.02x. Split this paragraph into pre-fix and post-fix comparisons.>>>
...
<<<CRITIQUE: This is important enough to appear earlier, likely before Table 1. A reviewer will otherwise read several pages of pre-fix performance before learning the fast path was algorithmically defective.>>>
...
<<<CRITIQUE: The number conflicts with Table 1 and `pipeline_run_20260521.json`, which list 708.32 s total and 700.73 s forecast-only. Decide which metric this sentence means. Also rephrase "3x above the 4-8x target": 23.02x is about 2.9x the upper end of 8x, not simply "3x above" the range.>>>
...
<<<CRITIQUE: "Architecture is correct" is too strong. The evidence supports device residency, restart continuity, partial invariants, and improved localization, but the boundary-width, land-state, theta guard, and skill failures mean the architecture is still under validation. Use "architecture remains viable" or "systems invariants remain intact".>>>
...
<<<CRITIQUE: Use 23.02x here for the current corrected-physics system, then mention 50.20x only as the invalid/diagnostic fast path. Otherwise the discussion repeats the abstract's main overclaim.>>>
...
<<<CRITIQUE: Stale after RCA/fix: radiation cadence was disabled in the pre-fix namelist, then changed to 180 in the post-fix path. Surface coupling also received a partial fix. Rewrite this limitation around remaining issues: theta guard saturation, frozen land/surface state, boundary width=1 versus WRF spec_bdy_width=5, single-case validation, and skill still outside tolerance.>>>
...
<<<CRITIQUE: Good honesty note for internal work, but a submission draft cannot cite an unresolved TODO. Resolve bibliography and remove this sentence before public release.>>>
...
<<<CRITIQUE: Public URL, commit hash, environment manifest, and lightweight verification script are not optional for this paper's central "proof-object discipline" claim. Keep this section as a submission blocker until populated.>>>
...
<<<CRITIQUE: Promote this from "should" to an acceptance criterion for publication readiness. The paper's method claims machine-auditable evidence, so the paper bundle needs at least a lightweight audit command.>>>
...
<<<CRITIQUE: Root-cause analysis is no longer fully open; two RCA sprints and a partial fix landed. Update this to "remaining skill closure, release-manifest pinning..." and list the specific unresolved technical defects.>>>
