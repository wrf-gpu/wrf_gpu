"""WDM6 (mp_physics=16) JAX port vs WRF savepoint parity gate.

Gates gpuwrf.physics.microphysics_wdm6 against the real WRF module_mp_wdm6.F
Fortran scheme via the proofs/v060_wdm6 savepoints (NOT a JAX self-compare).
The prognostic mass state + surface precip are gated against the canonical
fp32 WRF oracle; the double-moment NUMBER fields (Nn, Nc, Nr) are gated against
the fp32 oracle with a slightly looser relative tolerance (cube-root lamda
inversions + 1e1..1e10 dynamic range); the categorical-floor effective-radius
diagnostics are gated against the fp64 oracle (see
proofs/v060_wdm6/run_wdm6_parity.py for the full rationale).

Run CPU-only:
  JAX_PLATFORM_NAME=cpu pytest tests/test_wdm6_savepoint_parity.py
"""
import json
import os
...
from gpuwrf.physics.microphysics_wdm6 import (  # noqa: E402
    wdm6_run,
    wdm6_physics_tendency,
)

HERE = os.path.dirname(os.path.abspath(__file__))
PROOFS = os.path.abspath(os.path.join(HERE, "..", "proofs", "v060_wdm6"))
...
# Predeclared tolerances (must match proofs/v060_wdm6/run_wdm6_parity.py).
TOL = {
    "t_abs": 1.0e-2,
...
def _run(d, cid):
    ...
    args = [_col(d, k)[None, :] for k in
            ("T_IN", "QV_IN", "QC_IN", "QR_IN", "QI_IN", "QS_IN", "QG_IN",
             "NN_IN", "NC_IN", "NR_IN", "DEN", "P", "DELZ")]
    slmsk = np.array([SLMSK[cid]], dtype=np.float64)
    out = wdm6_run(*[jnp.asarray(a) for a in args], s["DT"], jnp.asarray(slmsk))
...
def test_physics_tendency_contract():
    """wdm6_physics_tendency returns a valid frozen PhysicsTendency."""
    d32, _ = _load(1)
    s = d32["scalars"]
