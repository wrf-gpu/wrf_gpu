...
def test_interfaces_self_check_and_scheme_specs_cover_v060_options() -> None:
    assert_interfaces_consistent()
    # 25 single-option specs (8 microphysics incl. Purdue-Lin + WSM3/WSM5 + 6 PBL
    # incl. BouLac + MRF(99) + 4 surface-layer + 5 cumulus incl. BMJ cu=2 + 2
    # land-surface) + 4 radiation variants (RRTMG LW/SW under option 4, classic RRTM
    # LW + Dudhia SW under option 1).
    assert len(SCHEME_STEP_SPECS) == 29
