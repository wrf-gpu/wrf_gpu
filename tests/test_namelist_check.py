...
def test_unsupported_selected_option_raises_actionable_error() -> None:
    ...
    assert "physics.mp_physics domain 2 selected 6" in message
    assert "supported values: 0, 8" in message
    assert "physics.cu_physics domain 1 selected 1" in message
    assert "KF(1) is not implemented on this branch" in message
    assert "Action:" in message


def test_registry_records_supported_active_suite() -> None:
    assert SUPPORTED_OPTIONS["mp_physics"].supported_values == frozenset({0, 8})
    assert SUPPORTED_OPTIONS["bl_pbl_physics"].supported_values == frozenset({0, 5})
    assert SUPPORTED_OPTIONS["sf_sfclay_physics"].supported_values == frozenset({0, 5})
    assert SUPPORTED_OPTIONS["sf_surface_physics"].supported_values == frozenset({0, 4})
