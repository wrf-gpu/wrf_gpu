...
- In h10 same-state dynamic localization, do not compare JAX directly against
  ...
  It is not history-aligned for `P/V/W`; bridge the pressure/rho/post-RK refresh
  path before or around `after_all_rk_steps`, then compare JAX at the accepted
  green post-marker or the next proven green refresh surface.

Reviewer Status:

Pending. Do not apply to stable memory until the post-RK refresh sprint confirms
whether this remains a general recurring gotcha or can be superseded by a more
specific cadence/root-cause lesson.
