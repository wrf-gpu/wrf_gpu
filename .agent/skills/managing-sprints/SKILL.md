...
- **MODEL POLICY OVERRIDE (principal directive 2026-06-11, sits ABOVE every older model assignment in this file while in effect; when they conflict, this block wins until the principal lifts the GPT-5.5 freeze):**
  ...
  - **GPT-5.5 / codex is OFF today — do NOT dispatch any GPT-5.5 work.** Token budget is exhausted; the principal will signal when it is back. Every role this file previously assigned to GPT-5.5 xhigh (verifier, critic, debugger, residual-fix, plan-critique, cross-model debug, management review) is filled by **Opus 4.8 xhigh** today.
