...
- **Cross-model debug cadence:** for a complex correctness bug, after two GPT/debug sprints on the same problem fail to prove a fix or leave the conclusion methodologically uncertain, dispatch one Opus xhigh critic/debugger to challenge the method, hypotheses, evidence chain, and candidate bug itself before the manager commits to the next conclusion. Use this as a targeted escalation, not routine double-agenting.
