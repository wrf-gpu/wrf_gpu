...
T3 status: microphysics(WSM7 column-kernel/oracle PASS but operationally fail-closed pending qh leaf), cumulus(oracle-infra, kernels carry-over), pbl(MRF op; ShinHong/QNSE carry-over), surface+lsm(2 sfclay op; slab ref-only; RUC carry-over). RUNNING: aacae3=T3-radiation (CPU), 2way+GWD-retest (GPU, hr2).
...
RUNNING: 9/3km(max-dom=2) 2-way+GWD 24h gate (GPU, #5 capability closure at fitting res); TOST-wrfbdy-fix (CPU). CLOSEABLES remaining: TOST n=15 (unblocking), #7 GPU A/B, mp-batch2 (WDM5+qh-leaf), slab-hook, sub-jit, CPU-flock. DOCUMENTED carry-overs (allowed): WSM7/WDM7 qh State/dynamics/I-O leaf, cumulus JAX kernels, CAM/NUWRF/GFDL radiation, Shin-Hong/QNSE, RUC-LSM (all multi-thousand-LOC); 2way+GWD+1km+24h + multi-hardware = 32GB-HW-limits.
...
DOCUMENTED CARRY-OVERS (principal-allowed): cumulus JAX kernels (2400-5300 LOC), CAM/Goddard-NUWRF/GFDL radiation (8-12k LOC), RUC-LSM (7516 LOC), Shin-Hong/QNSE PBL — all have de-risked oracle-infra; qh-State-leaf (→WSM7/WDM7 operational; WSM7 is currently column-oracle-proven but recognized_fail_closed) + slab-LSM-hook = focused future State sprints; 2way+GWD+1km+24h + multi-hardware = 32GB-HW-limits; #7 24h wind-skill closure = open research (lever landed, not closed).
...
**2026-06-08 ~15:14 — WDM5 MERGED** (672cb478): mp=14 operational, 6/6 pristine-WRF fp64 oracle PASS, clean 3-way (no conflicts, trunk only had doc commits since base), spec-count 36→37, 56 catalog tests green. v0.13 OPERATIONAL microphysics now: Thompson(8,default), Kessler(1), Lin(2), WSM3(3), WSM5(4), WSM6(6), Morrison(10), WDM6(16), **WDM5(14 NEW)**; WSM7(24)=column-oracle PASS but recognized_fail_closed pending qh leaf.
...
**2026-06-08 ~15:30 — BIG WAVE (trunk @ 0b917ea6, spec-count 37):**
 ...
 ✅ Agent O oracle-integrity audit MERGED (798704d5): 19 oracles, 0 BLOCKER/0 MAJOR/2 MINOR; 0 self-compare, 0 hidden-clamp, 3 bit-identity exact, 0 ref-only-as-operational; sha256 of every pristine module verified. VERDICT: proof corpus HONEST enough to ship. MINOR fix lanes M-1 (cumulus ref-only source-checksums) + M-2 (clear-sky absolute tolerance ceiling) identified; **manager remediation landed 2026-06-08** (`proofs/v013/t3_cumulus_source_checksums.json`; `proofs/v013/clearsky_radiation.json` now asserts fixed 10 W/m² clear-sky RMSE ceiling).
 ✅ GPT#1 impl-review MERGED (526f4d73): 2 MAJOR fixes (ra_sw/ra_lw=0 disabled-radiation no-op + zeroed diag; slab gpu_runnable=False). 75 tests green. Issues flagged → A (MRF+sf=3/91 diag mismatch, MAJOR) dispatched to Opus-max debug ae361b74; B (WSM7 mp=24 "ref-only" vs actual fail-closed — wording mismatch) closed by manager doc/comment alignment: WSM7 is column-oracle-proven but `recognized_fail_closed` until qh State/dynamics/I-O exists; namelist-doc already correct.
...
REMAINING TO RELEASE: smoke rc=0→full TOST n=15 (--resume); A1/A2 results; A4-A7 results; merge Issue-A fix + Agent F test module; #7 rad_rk_tendf A/B (GPU). THEN fill 18 placeholders + #5/#7/TOST/validation numbers + GPU-scalable positioning + KNOWN_ISSUES fill-list → gap-critic(Opus+GPT) → tag v0.13.0 → push wrfgpu HEAD:main+tag(home=latest).
...
ETA tag+push: tomorrow ~07:00-09:30 (n=15 pass + merges + packaging + gap-critic). PARALLEL CPU now/next-tick: merge Tiedtke b9420aae + canary-stats e96a16ef + Paper→local publish/; A5 matrix fix; Issue-B; draft release docs. TAG GATED ON n=15 rc=0 + equivalence PASS.

**2026-06-08 ~18:11 — loop tick (marathon healthy, GPU 74%, case 1/15).** Parallel non-src lanes landed: canary-stats MERGED (487fe178, scripts/canary_stats + proofs + report); Paper v0.14 extracted to LOCAL untracked publish/ (paper_v014_draft.md + claim_audit.md — ready for principal proofread). DEFERRED to post-GPU-phases (avoid half-merged src reads while cases spawn): Tiedtke src merge (b9420aae), Issue-B WSM7 wording, A5 matrix fix. trunk @ 487fe178. Marathon validating stable src @ 15a207b6-equiv (canary-stats was non-src so default path unchanged).

**2026-06-08 ~23:00 — Manager closes oracle-audit minors M-1/M-2 (CPU-only; no model-code change).** M-1: `proofs/v013/t3_cumulus_oracle.py` now emits `proofs/v013/t3_cumulus_source_checksums.json`, pinning pristine-WRF cumulus source files plus every present reference savepoint; New-Tiedtke and KIM-SAS are 5/5 pinned, Grell-3D remains honestly 0/5 not-built. M-2: `proofs/v013/clearsky_radiation.py` now requires both the original all-sky-floor-relative tolerance and a fixed **10 W/m² absolute clear-sky RMSE ceiling**; regenerated `proofs/v013/clearsky_radiation.json` is `OVERALL_PASS=true`, `A_oracle_absolute_rmse_pass=true`. TOST unaffected: proof/docs only, no `src/` diff since the TOST launch commit.

**2026-06-08 ~18:30 — PRINCIPAL 4 ITEMS:**
**#1 MARATHON DURABILITY + GPU-HANDOVER PROTOCOL.** Per-case proofs land DURABLE in proofs/v0120/powered_tost_n15/pipeline_proofs/<case>/ (repo dir → survives reboot); GPU wrfout outputs are in /tmp (lost on reboot, but NOT needed — each case's SCORED numbers (tier4_rmse, station_scores, validation_summary) are saved durably per-case). `--resume` keys on durable case_<RUN_ID>.json → on crash/reboot ONLY the in-flight case is lost; completed cases skipped. ADDED to loop: COMMIT each new case proof to git as it lands (logged case-by-case + pushable). GPU-HANDOVER PROTOCOL (principal needs GPU ~2h, ~20:10): on principal signal "I need the GPU now" → watch for the current case's case_<RUN_ID>.json to land (finish current case) → kill the n=15 driver (NEXT case not started; --resume-safe) → free GPU + confirm → WAIT for principal "continue" → relaunch `--resume` (skips done cases). Resume is MANUAL (principal's call), not auto.
**#4 CODEABLE-GAP TRACKER (from NCAR-post "What it isn't" — all codeable/debug gaps explicitly roadmapped; FUNDAMENTAL excluded):**
 - EXCLUDED (FUNDAMENTAL, never): bitwise identity to Fortran — different solver type (GPU-native JAX vs Fortran); permanent property, not a gap.
 - ALREADY TRACKED: full physics scheme long-tail MP/CU/PBL/RA/LSM (Tier3); 2-way feedback full 24h-equivalence (line 21); multi-GPU real-HW NCCL/throughput (S1, line 24, needs HW); region/season breadth (v0.14 B-campaign).
 - NEWLY ADDED to v0.14→v1.0 roadmap (codeable): (a) shallow convection schemes; (b) complete dynamics option matrix — PD/monotonic scalar advection operational-default (fn merged 584037d, wire+default), full diffusion/damping matrix, adaptive timestep, DFI; (c) I/O completeness — full wrfout variable set, complete namelist parse/validate coverage; (d) FDDA/DA (grid/spectral/obs nudging), offline ndown, moving/vortex-following nests — codeable big-features (were "deliberately out-of-scope" for v0.13; reclassified as planned v0.14→v1.0 work per principal); (e) stochastic physics (SKEB/SPPT) — codeable, was out-of-scope, now roadmapped.
 - HW-LIMIT (not codeable on one card; multi-GPU resolves): 1km+24h+2way+GWD combined config = 32GB-VRAM-marginal.

**2026-06-08 ~19:13 — loop tick.** A5 (multicfg operational coupler smoke) GREEN: a5_v3.rc=0, 21/21 RUN PASS + 1/1 FAIL_CLOSED OK (pbl_acm2→sf1 fixed; MYJ excluded as harness-scope w/ documented real coverage in test_v013_myj_janjic_operational.py). Marathon: 1/15 TOST cases banked (case_20260429... durable+committed), case 2 (20260430) forecasting ~40min in. VRAM-probe agent a151011b running (CPU 8-15) — measuring 641×321 1km fp64 footprint for the principal's ML-training target geometry (answer A: does it fit 32GB live-nest vs d03-replay + max-fitting grid). PRINCIPAL ML-target note: 9/3/641×321-1km live nest likely exceeds 32GB (fp64 + JAX intermediates + resident radiation g-points; live anchor=~20GB for a tiny 9/3km nest); d03-replay is the realistic route; CPU-WRF compare day for the NEW geometry needs canairy WPS (other project).

**2026-06-08 ~19:18 — VRAM-PROBE RESULT (ML-target 641×321 1km) — MEASURED, HIGH confidence (e2a7aa1f, .agent/reviews/2026-06-08-opus-1km-target-vram-measurement.md):**
 641×321×50 1km fp64 does NOT fit 32GB — ~90 GiB BOTH live-nest and d03-replay (~3× over). Largest 1km fitting 32GB ≈ 59.5k cols (~243×243 or ~345×172). Prod d03 94×76 = 3.1 GiB (fits, consistent w/ GREEN nested-1km gate).
 ROOT CAUSE = the fp64 RRTMG g-point per-step TRANSIENT materialized over the FULL ncol batch (one taumol (ncol,nlay,16,16) fp64 = 19.6 GiB at target; LW binding). Resident State only 2 GiB → NOT the constraint. THIS is why we're ~8× the CPU's 11GB: CPU-WRF loops radiation column-by-column (never holds the full g-point array); we batch all columns in fp64. chunk=1 (SW−88.6%/LW−43.6%) only buys 1.8× — can't close 3× alone.
 → NEW v0.14 CODEABLE LANE (the fix for big-grid VRAM + the ML-training-data target): **column-tile the RRTMG SW+LW drivers** (lax.scan over ~60k-col tiles, same pattern as the shipped band-tiling) → caps the radiation transient → makes 641×321 fit at a radiation-step wall-clock cost (state is tiny). Alternatives: fp32-gate radiation (~halve, needs ADR), multi-GPU column sharding (HW). Until then the ML-target needs a SMALLER 1km grid (~243×243 / ~345×172) OR the column-tiling lane.

**2026-06-08 ~19:40 — PRINCIPAL: memory-efficiency is a v0.13 BLOCKER for core modules ("near-optimum, not wasteful with RAM"). TOST STOPPED at 2/15 (durable+committed: 20260429, 20260430; --resume continues later) to do the memory fix RIGHT + restart TOST clean on fixed code.** Principal hibernating in ~30-40min → GPU freed now.
MEMORY-FIX WAVE (4 agents, all CPU/analysis — survive hibernate; restart TOST after fixes validated + GPU back):
 (1) Opus-max: RADIATION COLUMN-TILE FIX — rrtmg_sw.py + rrtmg_lw.py, lax.scan over column tiles (mirror the shipped band-tiling), cap the per-step g-point transient (the 19.6GiB taumol), BIT-IDENTICAL (max_rel=0.0, oracle-verified vs pristine), tile-size knob, default bit-unchanged. The decided v0.13 fix.
 (2) Opus: EMPIRICAL memory-MAP — CPU peak-RSS per scheme + per dycore-phase on a real-ish nested config (load + few steps), rank the next-biggest consumers AFTER radiation.
 (3) GPT-codex (tmux): ANALYTIC memory-MAP — static accounting of ALL materialized fp64 arrays (dycore + physics + coupling), per-module, cross-checked vs existing GPU proofs; find every wasteful spot.
 (4) GPT-codex (tmux): fp32-ACOUSTIC FEASIBILITY — is an fp32 alternate acoustic-solver kernel possible (stable on GPU) or a mathematical necessity? why does it blow up? if feasible → roadmap + implication analysis. The ~50% VRAM lever (principal: worth more than rare schemes).
"Memory efficiency < compute efficiency in priority, but core modules must be near-optimum." TOST verdict still the tag-blocker; memory fixes now part of v0.13.
