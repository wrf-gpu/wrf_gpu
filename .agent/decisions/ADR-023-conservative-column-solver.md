# ADR-023 — Conservative Column Solver (SUPERSEDED-PROVISIONAL)

**Status**: **SUPERSEDED-PROVISIONAL** as of 2026-05-24 per external consultation + manager reflection `.agent/decisions/manager-reflections/PLAN-REFLECTION-2026-05-24-post-consultation.md`. The minimalist-carry thesis was scientifically defensible, but S2.1-redo real Gen2 d02 1h baseline (commit `4b97743`) produced T2 RMSE 136.9 K (218× Gen2 noise floor), 17 B sanitized nonfinites, and θ pinned to sanitizer caps. The subsequent operator bug-hunt (`e036ef8`, 7 single-suspect A/B toggles with sanitizer-bypass) verdicted `NO-BUG-LOCALIZED`. ADR-023's body is kept as scientific reference, NOT as the production architecture. The replacement architecture is ADR-025 (TO BE DRAFTED during M6B0): WRF small-step bottom-up port under savepoint parity. Do not extend ADR-023 with new stabilizers; instead build the WRF savepoint harness and rebuild operator-by-operator under hard per-step parity.

**Prior status (now historical)**: PROPOSED — ratified 2026-05-23 02:10 UTC after round-2 critic `RATIFY-ADR-023` + prototype `PASS_WARM_BUBBLE_600S` + R7 GREEN. The production-grade sprint folded the critic fixes into code and tests, but reviewer `b2f7a05` rejected the path split. The public-scan unification sprint routed the public nonhydrostatic scan through the MPAS recurrence, but the unified warm-bubble path did not meet the 600 s warm-bubble acceptance targets, and the real d02 evidence above invalidated the thesis at 1h.
**Date**: 2026-05-23
