...
|---|---|---|
| Grid-cell parity | Active. MYNN source-output deficit, MYNN surface first-call semantics, WRF `LANDUSE.TBL`-backed `TSK/ZNT/MAVAIL`, WRF `phy_prep` thermodynamic inputs, local `SFCLAY1D_mynn` output algebra, MYNN dry-theta/phy_prep source coupling, WRF surface/land flux handoff localization, and the JAX Step-1 NoahMP-disabled configuration gap are fixed/proven or bounded. Fable/Mythos closed NoahMP land-tile energy/HFX as the primary blocker: `proofs/v014/noahmp_land_tile_energy_closure.*` proves the energy solve is WRF-exact under WRF inputs and the production `sfctmp` path now decouples moist theta to dry theta before Exner conversion. Strict Step-1 remains red against pinned one-run WRF truth at max_abs `1489.5135568470864`, RMSE `12.146876720723487`; the worst cell is water, outside NoahMP. New audit `proofs/v014/moist_theta_physics_consumer_audit.*` expands the θ_m boundary to multiple physics adapters; state/LBC/dycore keep moist theta, physics adapters require dry views and moist writeback. Secondary RRTMG GLW/RTHRATEN residual is localized, not closed: `proofs/v014/rrtmg_step1_forcing_parity.*` names the remaining boundary as clear-sky derived RRTMG optical/gas/top-buffer profile or downstream kernel boundary; no production fix is proven. | Close the same moist-theta-to-dry-temperature boundary in `surface_layer.py` / sfclay-MYNN water path with MYNN re-validation; then close RRTMG with a temporary WRF RRTMG forcing hook for derived LW/SW profiles, fluxes, and heating arrays, or record a formal demotion before release. |
...
1. Close the surface-layer/sfclay-MYNN water-path moist-theta-to-dry-temperature
   boundary with proof and MYNN re-validation.
