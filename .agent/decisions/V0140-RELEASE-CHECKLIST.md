...
| Canary field parity | L2 d02 has retained CPU-WRF 72h truth: 15 complete backfill cases in `/mnt/data/canairy_meteo/runs/wrf_l2_backfill_output`, each with 73 d02 frames. The selected gate case is `20260501_18z_l2_72h_20260519T173026Z`, because it is already the h1 field-falsifier case and has WRF `rc=0` backfill provenance. The post-EOS h1 adjudication verdict was `PROCEED_72H_GATES`. A detached Canary d02 72h GPU field-gate is running at `/mnt/data/wrf_gpu_validation/v014_canary_d02_72h_20260610T142426Z` with resource CSV logging, but h08/h10 intermediate grid compares already report `FAIL` with dynamic `PSFC/MU/P/PH` drift. Fable high is analyzing this in tmux `0:3`; report path `.agent/reviews/2026-06-10-v014-fable-canary-h08-drift-analysis.md`. | Do not call Canary green unless the final/next compare proves the drift bounded or root-caused. Let the run reach h24 unless Fable returns a strong stop/fix recommendation earlier; inspect h24 compare, then decide stop/continue/fix. If fixed or formally bounded, rerun the field gate and only then launch Switzerland GPU next. |
...
5. Run Canary L2 d02 72h GPU-vs-CPU field-parity/stability with resource CSVs.
   Current detached run:
   `/mnt/data/wrf_gpu_validation/v014_canary_d02_72h_20260610T142426Z`.
   h08/h10 intermediate compares are red; h24 plus the Fable analysis are the
   immediate stop/continue/fix decision points.
6. Run Switzerland/Gotthard 72h GPU-vs-CPU field-parity/stability with resource
