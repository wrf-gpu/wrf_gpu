...
| Switzerland/Gotthard | CPU72 truth is complete at `/mnt/data/wrf_gpu_validation/v014_switzerland_72h_cpu_20260610T122909Z/run_cpu`: 73 `wrfout_d01_*`, `rc=0`, `SUCCESS COMPLETE WRF`, last-frame finite PASS. Timing: total wall `2906.3 s`, mainloop `2887.6 s`, 24 dmpar MPI ranks. Resource CSVs are under `/mnt/data/wrf_gpu_validation/v014_switzerland_72h_cpu_20260610T122909Z/resources`; peak 24-rank `wrf.exe` RSS sum `12636.176 MiB`. LBC-clock bug fixed and proven. HPG native-face mismatch fixed but refuted as blocker. Real-case `rhs_ph` + edge-faithful stage omega + WRF dycore constants are merged/proven and improve residual `-27.697 -> -21.883 Pa/cell/h`, but excess outflux only moves ~4%, so blocker remains open. | Do not rerun 72h yet. Continue from stage-3/end-of-step wrapper cadence and residual lateral-band amplifier in `.agent/sprints/2026-06-11-v014-switzerland-acoustic-substep-continuation/manager-handoff.md`; close the h36 storm-state short gate first, then rerun Switzerland 72h GPU with resource CSVs and atlas. |
...
11. Close Switzerland strong-flow dry-dynamics residual with the h36 short gate:
   ...
   Continue from stage-3/end-of-step wrapper cadence and residual band
   amplifier; do not start the 72h gate until the h36 short-gate residual
