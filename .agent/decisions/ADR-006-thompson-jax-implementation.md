...
Decision: Implement the M5-S1 Thompson column as a single JAX source/sink kernel with sedimentation removed, driven by an analytic WRF-source-mapped Path-B fixture. The public API is `step_thompson_column(state, dt, *, debug=False) -> state`, where `state` is the `ThompsonColumnState` pytree carrying `qv`, `qc`, `qr`, `qi`, `qs`, `qg`, `Ni`, `Nr`, `T`, `p`, and `rho`.
...
- Saturation setup: water and ice saturation ratios, supersaturation flags, diffusion/viscosity, `ocp`, and `lvap` setup are mapped from lines 2040-2063. The exact RSLF/RSIF polynomial helper formulas are mapped from lines 5444-5490.
- Warm-rain and cloud adjustment: cloud-water condensation/evaporation with a fixed three-iteration Newton update is mapped from lines 3456-3556. Rain evaporation shape is mapped from lines 3561-3633. M5-S1 keeps the water-conserving source/sink form but does not map aerosol-aware CCN activation tables.
- Freezing/melting: instant cloud-ice melting and cloud-water homogeneous freezing are mapped from lines 4000-4038. The final state floors and number concentration constraints are mapped from lines 4039-4152.
- Tendency/update bookkeeping: source/sink tendencies and latent heating structure are mapped from lines 3024-3273. M5-S1 keeps the water-conserving source/sink partition, fp64 arithmetic, and Thompson floors; it intentionally uses compact analytic rates in place of WRF lookup-table collection integrals.
...
This sprint proves the JAX/fixture/tier-validation pipeline and a real branchy Thompson-shaped source/sink column, but it is not full WRF Thompson. The largest scientific gaps are the WRF lookup-table collection terms, aerosol-aware activation/scavenging, graupel volume/hail details, and sedimentation. Sedimentation must be a dedicated follow-up sprint before any precipitation fallout claim is made.
