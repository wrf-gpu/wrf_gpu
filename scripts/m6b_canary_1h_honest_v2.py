#!/usr/bin/env python
"""M6b retry: honest 1h Canary d02 operational-mode acceptance.

The retry differs from the first M6b attempt in two binding ways:

* it runs after the promoted operational carry expansion; and
* it applies the corrected per-level theta bounds: lower 30 levels
  200-400 K, upper 14 levels 250-700 K.
"""
...
from gpuwrf.integration.d02_replay import _reference_fields, _surface_fields
...
def _d2h_inheritance() -> dict[str, Any]:
    if D2H_WARMED_MEMO.exists():
        text = D2H_WARMED_MEMO.read_text(encoding="utf-8")
        lowered = text.lower()
        go = "go" in lowered and ("0 d2h" in lowered or "d2h count: 0" in lowered or "d2h=0" in lowered)
        return {
            "status": "GO" if go else "UNKNOWN",
            "source": str(D2H_WARMED_MEMO.relative_to(ROOT)),
            "summary": "warmed D2H memo present; parsed as GO" if go else "warmed D2H memo present but not parsed as GO",
        }
    return {
        "status": "MISSING",
        "source": str(D2H_WARMED_MEMO.relative_to(ROOT)),
        "fallback_source": str(D2H_GREP_MEMO.relative_to(ROOT)) if D2H_GREP_MEMO.exists() else None,
        "summary": "No warmed re-capture memo/proof is present in this checkout; cannot close the transfer-cleanliness inheritance gate.",
    }
...
def run_one(run_id: str, *, hours: float) -> dict[str, Any]:
    ...
    final_bounds = _bounds_for_state(forecast_state, step=int(round(hours * 3600.0 / namelist.dt_s)), lead_seconds=hours * 3600.0)
...
def run_acceptance(run_ids: tuple[str, ...], *, hours: float) -> dict[str, Any]:
    ...
    elif blocker is None and d2h["status"] != "GO":
        blocker = "D2H_INHERITANCE_MISSING"
    ...
    payload = {
        "artifact_type": "m6b_honest_1h_canary_retry_acceptance",
