"""F7N official idealized-case proof run -> proofs/f7n.

Straka 900s + Skamarock warm bubble 500s with the F7N dycore (WRF-faithful
vertical-momentum-advection sign fix in flux_advection._vertical_flux_div_3).
Writes JSON + verdict to proofs/f7n.
"""
...
def main():
    ...
    if which in ("straka", "both"):
        ...
        r = run_density_current_case(proof_dir="proofs/f7n", require_gpu=True)
    ...
    if which in ("bubble", "both"):
        ...
        r = run_warm_bubble_case(proof_dir="proofs/f7n", require_gpu=True)
