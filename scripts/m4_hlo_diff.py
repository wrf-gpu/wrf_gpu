...
from gpuwrf.dynamics.step import step, step_stripped_reference  # noqa: E402
...
def _normalize(text: str) -> str:
    ...
    text = re.sub(r"step_stripped_reference", "step", text)
...
def main() -> int:
    ...
    stripped = compiled_text(step_stripped_reference.lower(state, tendencies, grid, 2.0, n_acoustic=4).compile())
    write_hlo(ART / "dycore_step_production.txt", prod, SCRATCH / "dycore_step_production_full.txt")
    write_hlo(ART / "dycore_step_debug_stripped.txt", prod, SCRATCH / "dycore_step_debug_stripped_full.txt")
