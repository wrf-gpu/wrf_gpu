"""F7M official idealized-case proof run -> proofs/f7m.

Straka 900s + warm bubble 500s with the F7M dycore (flux-form momentum advection
+ deformation-tensor constant-K momentum diffusion).  Writes JSON + verdict.
"""
...
def main():
    ...
    if which in ("straka", "both"):
        ...
        r = run_density_current_case(proof_dir="proofs/f7m", require_gpu=True)
    ...
    if which in ("bubble", "both"):
        ...
        r = run_warm_bubble_case(proof_dir="proofs/f7m", require_gpu=True)
