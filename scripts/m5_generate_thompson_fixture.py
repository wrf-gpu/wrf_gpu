...
def _manifest_variables(shape: tuple[int, ...]) -> list[dict[str, Any]]:
    ...
    for name in ("qv", "qc", "qr", "qi", "qs", "qg"):
        variables.append(_variable(f"output_{name}", "kg kg-1", shape, 2.0e-4, 1.0, "Attempt-5 carry-forward tolerance while strict ADR-005 table-proxy residuals are deferred to M5-S1.x"))
    for name in ("Ni", "Nr"):
        variables.append(_variable(f"output_{name}", "kg-1", shape, 2.0e6, 10.0, "Attempt-5 carry-forward tolerance while strict ADR-005 table-proxy residuals are deferred to M5-S1.x"))
    variables.append(_variable("output_T", "K", shape, 2.0, 0.02, "Attempt-5 carry-forward tolerance while strict ADR-005 process/table residuals are deferred to M5-S1.x"))
    return variables
...
def write_fixture() -> dict[str, Any]:
    ...
    manifest = {
        ...
        "source": "wrf-derived",
        "source_commit": f"wrf-thompson-via-fortran-harness sha256={harness_sha}; module_mp_thompson.F.pre",
        "wrf_version": "v4.7.1",
