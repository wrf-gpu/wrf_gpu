...
These transitions are tabulated per stage in §3.5 and `publish/tables/ai_process_ledger.md` (stage (a) Opus 4.7 manager / GPT-5.5 frontrunner → stages (b)–(f) Opus 4.8 manager / Opus 4.8-max frontrunner, with the verifier moving from every-sprint to every-milestone and GPT-5.5/Gemini reactive). **[PLACEHOLDER: a model-role timeline figure (stage on x-axis, role band per model), to render to `publish/figures/model_role_timeline.png` from the git-history accounting in `publish/tables/effort_accounting.md`; the existing `publish/figures/timeline.md` spec is M7-era. — `missing_elements.md` (i). Figure pending; the text and §3.5 table carry the content.]**
...
| d03 daytime surface-flux (HFX) warm bias mistaken for a dycore/boundary problem | The bare `sfclayrev` port reused momentum roughness for the heat profile, while corpus MYNN uses a smaller land thermal roughness → HFX ≈ 4× too high, T2 ≈ +3.6 K | External column-oracle parity (corpus-WRF inputs → GPU surface layer → vs same-step WRF, not a self-compare) | MYNN land thermal-roughness ported: land HFX over-flux 4.22× → 2.30×, T2 land bias +3.6 K → +1.2 K; 24 h re-confirmation in progress, strict 1 km gate not yet re-passed (honest, §6.3) |
...
- **Tier 4 — real-case skill + persistence baselines.** RMSE versus same-workstation CPU-WRF at multiple leads on real Canary cases, with a persistence baseline (1 − GPU_RMSE/persistence_RMSE) as the skill discriminant. The persistence baseline is what exposed the wind deficiency that finite-forecast checks missed (§6.5). A formal *statistical* equivalence test — a predeclared paired TOST (ADR-029) — is the binding seasonal-equivalence gate; the v0.1.0 corpus supports it only as an **underpowered, single-season (spring trade-wind, n = 3 distinct MAM days) descriptive check, which we never call "seasonal."** The full ≥15-case (target ≈27–30) seasonal TOST is an explicit v0.2.0 release gate; the harness, scorer, and predeclared margins are already built and self-tested (`proofs/m20/`), and the corpus backfill is the only missing piece (§8).
- **Systems validation.** A length-independent device-residency check, a (counted) in-loop D2H transfer audit, restart continuity, warm-run repeatability, and profiler provenance, each bound to a proof object — several still in-progress for v0.1.0 (§6.4).

**[PLACEHOLDER: validation-pyramid figure — regenerate `publish/figures/validation_pyramid.png` from the M7-era `publish/figures/validation_pyramid.md` spec to reflect v0.1.0 status (Tier 1–3 green; Tier 4 d02 green / d03 marginal-with-HFX-fix-in-progress / TOST single-season-underpowered, full seasonal = v0.2.0). — `missing_elements.md` (i). Figure pending.]**
...
The companion gate table (`publish/tables/idealized_gate_summary.md`) is generated. **[PLACEHOLDER: idealized figure set — warm-bubble θ′/w panel and Straka density-current panel, to render to `publish/figures/warm_bubble_panel.png` / `straka_density_current_panel.png` from the PPM/plot outputs under `proofs/f7n/plots/` (already copied to `publish/figures/idealized/`), `proofs/wind/idealized_postfix/plots`. — `missing_elements.md` (i). Figures pending; the gate table carries the content.]**
...
The strict bounded gate is therefore still **`D03_1KM_BOUNDED_FAIL`** at the v5fix proof — T2 final-lead RMSE 3.01 K sits marginally above the 3.0 K threshold (24 h mean T2 RMSE 2.43 K; overnight ≈ 1.4 K) — even though this is a dramatic improvement over the pre-fix run (final-lead T2 6.76 K → 3.01 K, U10 7.41 → 3.49, V10 8.39 → 4.40) and U10/V10 are within their thresholds. The remaining residual is a **daytime surface-flux (HFX) warm bias shared across domains** (peak T2 mean-bias +2.97 K at 13z): the corpus L3 ran the MYNN surface layer (`sf_sfclay_physics=5`), which over land uses a thermal roughness length `z_t` (Zilitinkevich 1995, CZIL = 0.085) much smaller than the momentum roughness for the heat profile, whereas the bare `sfclayrev` port reused the momentum roughness for heat — making `psit` ≈ 4× too small, HFX ≈ 4× too high, and T2 ≈ +3.6 K warm over land.

**HFX fix — validated at the 1 h column oracle, 24 h confirmation in progress.** The MYNN land thermal-roughness term was ported for `psit`/`psit2`/`psiq`; driving corpus-WRF midday d03 column inputs through the corrected `sfclayrev` and comparing against same-step WRF (an *external-oracle* parity, not a self-compare) collapses the land HFX over-flux from ≈ 4.22× to ≈ 2.30× of WRF and the binding T2 diagnostic from +3.6 K to +1.2 K land bias (T2 all-cell RMSE 2.106 K → 0.827 K), with water-side fluxes and momentum unchanged [proof: `proofs/v010_validation/sfclay_hfx_oracle_parity.json`]. The residual ≈ 2.3× land HFX is attributed to Noah-MP surface-energy-balance coupling not reproducible by a standalone surface layer (P0-3/P1-4), not to the surface-layer port. **The full 24 h d03 re-run with this fix is in progress and is not yet a clean proof object — the current attempt is `D03_1KM_BLOCKED` (a GPU out-of-memory under shared-GPU contention, not a forecast failure)** [proof: `proofs/v010_validation/d03_summary_run24h_hfxfix.json`, status BLOCKED]. We state d03 precisely: *the boundary-pump bug is fixed, overnight d03 already reaches d02-quality, the HFX root cause is identified and fixed at the column-oracle level, but the strict 24 h 1 km gate has not yet been re-passed on the final code*; 1 km is therefore **not** in the positive v0.1.0 claim and is listed as a release gate (§8, `publish/VERIFICATION.md` row 5).

The d03 status table — pre-fix→post-fix bias collapse, v5fix final-lead scores vs persistence, and the per-lead persistence win/loss tally — is generated at `publish/tables/v010_d03_status.md`.
...
The companion tables are generated: `publish/tables/performance_current.md` (current roofline-grounded headline, replacing the stale `performance_evolution.md`) and `publish/tables/optimization_refutations.md` (the five measured-and-refuted levers, each with proof path and fidelity verdict). **[PLACEHOLDER: roofline figure — dycore AI 0.40, HBM/fp64 ridges, 5.3x-over-floor placement, to render to `publish/figures/roofline_dycore.png` from `proofs/perf/roofline_costonly.json` + `phase_breakdown.json`. — `missing_elements.md` (i). Figure pending; the performance table carries the content.]**

**Systems evidence** (`publish/tables/systems_invariants.md`). The current v0.1.0 d02/d03 pipeline reports a 24 h end-to-end speedup of **9.09×** against the derived CPU d02 denominator (pipeline wall 1794 s vs 16 305 s) [proof: `proofs/v010_validation/speedup_vs_cpu_24h.json`, PASS], consistent with the per-forecast-hour roofline framing once IC load, hourly writes, inventory, and scoring overhead are included. The 24 h coupled real-d02 run is **all-finite and physically plausible guards-OFF** at fp64 [`proofs/perf/coriolis_segscan_24h.json`], and **peak device memory is independent of forecast length** (10 211 MB after a full 24 h run ≈ 9 048 MB after one 180-step segment), confirming no per-step trajectory accumulation on device. The architectural no-in-loop-transfer invariant is enforced by the segmented host-loop, **but a *counted* v0.1.0 D2H-transfer proof object, the repeatability re-run, and the restart-continuity probe are still in-progress** — the audit harness exists (`proofs/perf/fusion_transfer_audit.py`) but emitted no committed count, and the pipeline `repeatability.json`/`restart_in_pipeline.json` are `NOT_RUN` (the `--repeat`/`--restart-at-hour` switches were not enabled for the scored run; *unmeasured*, not *failed*). These are tracked as release gates (`publish/VERIFICATION.md` rows 8, 11; `missing_elements.md` (ii)) and the historical M7 audit reported 0 copies / 0 bytes.
...
**(d) The d03 boundary-pump self-correction — and the bias it unmasked.** The 1 km nest blew up its surface bias (+6.8 K T2, ~2.5x wind bias). Validation against CPU-WRF localized it to a nested geopotential-boundary forcing term pumping the interior; a WRF-faithful fix collapsed the overnight bias to d02-quality and — critically — *exposed a separate, previously masked daytime surface-flux warm bias.* That residual was then itself localized by an external column-oracle parity (corpus-WRF inputs → GPU `sfclayrev` → vs same-step WRF): the corpus ran the MYNN surface layer, whose land heat profile uses a thermal roughness ≪ the momentum roughness, so the bare port's reuse of momentum roughness for heat made HFX ≈ 4× too high. Porting the MYNN land thermal-roughness term cut the land HFX over-flux ≈ 4.22× → 2.30× and the T2 land bias +3.6 K → +1.2 K; the 24 h confirmation is in progress and the strict gate is honestly *not yet re-passed* (§6.3) [proofs: `proofs/v010_validation/sfclay_hfx_oracle_parity.json`, `d03_summary_run24h_hfxfix.json`]. *Lesson: fixing one defect often unmasks another; the proof-object trail keeps both visible, and an external oracle pins the root cause even when the integrated re-run is still running.*

**[PLACEHOLDER: self-correction timeline figure — v0.0.1 over-claim → self-compare retraction → dycore F7 close → speedup-denominator correction → persistence-baseline wind gap → missing-Coriolis fix → d03 boundary-pump fix → HFX/MYNN thermal-roughness fix → current v0.1.0 status. To render to `publish/figures/self_correction_timeline.{md,png}` from this section + the §3.6 ledger. — `missing_elements.md` (i). Optional; the §3.6 ledger and §6.5 text carry the content.]**
...
**P1 — fidelity/robustness debts:** RRTMG topo-shading/slope-radiation and real lat/lon (P1-3); MYNN EDMF/cloud completeness, central to marine PBL and the residual wind skill (P1-4); Thompson parity debts — adaptive sedimentation substeps, cloud-water sedimentation (P1-5); explicit precision-policy proof gates (P1-8); gravity-wave drag if load-bearing (P1-7); positive-definite/monotonic scalar advection and boundary-order options (P1-6); data assimilation/FDDA only if a future namelist requires it (P1-1). The 1 km daytime surface-flux residual (§6.3) is the near-term P1-4/P0-3 item; its root cause (MYNN land thermal roughness) is fixed at the column-oracle level and the 24 h re-confirmation is the immediate d03 gate. **0.2.0 scalability:** single-node multi-GPU domain decomposition (S1), pre-designed via the frozen halo interface.

**Seasonal-equivalence TOST — an explicit, scoped v0.2.0 proof-promise.** The binding statistical-equivalence gate is a predeclared paired TOST (ADR-029) showing the GPU model's T2/U10/V10 RMSE equivalent to CPU-WRF at frozen 10 %-of-benchmark margins on a ≥15-case (target ≈27–30 for the 10 % MDE), multi-season ensemble. v0.1.0 does **not** make this claim: the reuse-only corpus collapses to **3 distinct usable MAM (spring) days** after output-purge, L2/L3 sibling de-duplication, and obs-window clipping, so a TOST on it would be both underpowered and single-season — we report it (if at all) only as an underpowered single-season *descriptive* check and never as "seasonal." What we *can* promise concretely, because the evidence already largely exists, is the cadence: the TOST harness, the station-paired scorer, and the predeclared per-variable margins are built and **self-tested** (a CPU-vs-CPU self-test reproduces the ADR-029 benchmarks to 0.00 paired delta) [proofs: `proofs/m20/{tost_design.json,tost_ensemble_runner.py,paired_tost_scorer.py}`, `selftest_verify_release.json`; plan `proofs/m20/tost_campaign_plan.md`]. The achievable n = 3 GPU run is ≈1.1 GPU-hours; a single-season n ≈ 15 MAM corpus needs ≈13 CPU-WRF backfill runs (≈68 CPU-hours ≈ a few background nights on cores 4–31, with the raw AIFS forcing already preserved for the dates) plus ≈5.5 GPU-hours of scoring. We therefore promise the **full single-season ≥15-case TOST at the v0.2.0 release within days of the backfill completing, executed by the same AI process that built the harness** — this is the one timing commitment in the paper, and it is a proof-promise (the machinery is already validated; only the corpus backfill is outstanding), not a generic future-work cadence. A genuinely *multi-season* TOST requires going-forward nightly retained-output capture (no cross-season forcing exists), which is a slower, calendar-bound v0.2.0 deliverable. Readiness detail: `publish/tables/tost_readiness.md`.

**Honest caveats on timing.** The validated core benefited from unusually clean oracles (analytic cases + WRF savepoints); the two items with the messiest validation — native init (P0-2) and prognostic Noah-MP (P0-3) — have error bars that skew high. We report a roadmap, not a schedule, and — with the single exception of the seasonal-TOST proof-promise above (where the machinery is already built and self-tested, so only a bounded corpus backfill remains) — we explicitly do not promise that unfinished items will be "finished within hours or days." Hardware portability is favorable: the pure JAX/XLA code recompiles for Hopper (H100/H200) with no source changes and should run faster (full-rate fp64, higher HBM bandwidth) on larger single-GPU domains; speedup-vs-CPU is hardware-specific and must be re-measured on that hardware (we have none).
...
- Real-case d03: `proofs/v010_validation/d03_summary_run24h_v5fix.json` (+ pre-fix `..._v4.json` for the self-correction row), HFX fix `sfclay_hfx_oracle_parity.json` + in-progress `d03_summary_run24h_hfxfix.json` (status BLOCKED); table `publish/tables/v010_d03_status.md`.
...
- Seasonal-TOST readiness (v0.2.0 proof-promise): `proofs/m20/{tost_design.json,tost_campaign_plan.md,tost_ensemble_runner.py,paired_tost_scorer.py,selftest_verify_release.json,seasonal_gap_assessment.md}`; table `publish/tables/tost_readiness.md`.
