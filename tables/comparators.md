...
Source discipline: prior-art values are taken from `publication/research_brief/english_brief.txt` as directed by the sprint contract; citation keys are from the frozen `paper/references.bib`. These rows are context, not normalized apples-to-apples benchmarks against this repository's 20260521 Canary d02 replay.
...
Current repository performance is tracked separately in `tables/performance_evolution.md` because its proof object is local measurement evidence, not a bibliographic prior-art citation.
