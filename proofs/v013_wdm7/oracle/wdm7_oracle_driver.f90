! =====================================================================
! v0.13 single-column WDM7 (WRF mp_physics=26) oracle driver.
!
! Drives the UNMODIFIED WRF phys/module_mp_wdm7.F:
!   wdm7init       (runs radar_init; double-moment warm rain + hail)
!   wdm72D         (the full DOUBLE-MOMENT 7-class hail microphysics, one
!                   column; called directly on a single column)
!   effectRad_wdm7 (effective radii for radiation; identical to effectRad_wdm6)
! on a prescribed single column (idim=1) and dumps the FULL input state
! (t,qv,qc,qr,qi,qs,qg,qh,Nn,Nc,Nr,den,p,delz,pii) and the FULL output state
! (t,qv,qc,qr,qi,qs,qg,qh,Nn,Nc,Nr after the call) plus surface accumulators
! (rain,rainncv,snow,snowncv,graupel,graupelncv,hail,hailncv,sr) and effective
! radii (re_cloud,re_ice,re_snow).
!
! WDM7 = WDM6 ice/snow/graupel + DOUBLE-MOMENT warm rain + a SEPARATE
! precipitating HAIL class. Hail is SINGLE-MOMENT (qh only; there is NO hail
! number). The number array ncr packs: ncr(:,:,1)=Nn (CCN), ncr(:,:,2)=Nc
! (cloud), ncr(:,:,3)=Nr (rain). The 2D entry wdm72D works on
! t(its:ite,kts:kte), q/den/p/delz(ims:ime,kms:kme), qci(:,:,1:2)=qc/qi,
! qrs(:,:,1:4)=qr/qs/qg/qh, ncr(:,:,1:3)=Nn/Nc/Nr.
!
! This is the GOLD oracle for the JAX port: it is the real Fortran WDM7
! scheme (module_mp_wdm7.F), not a re-implementation, so the JAX port cannot
! "self-compare". The init is the real wdm7init so every saved module constant
! (gammas, slope maxima, fall-speed prefactors incl hail, pidnc/pidnr/qck1) is
! exactly WRF.
!
! NOTE on dependency stubs: module_mp_wdm7.F `use`s module_mp_radar (only for
...
! UNMODIFIED phys/module_mp_radar.F (so refl10cm_wdm7 + radar_init resolve) and
! provide tiny no-op module_wrf_error (wrf_debug) + a tiny module_model_constants
! with the 3 RE constants. The microphysics tendencies (mass + Nc/Nr/Nn) are
! produced by the UNMODIFIED scheme; the reflectivity DIAGNOSTIC is not enabled
! (diagflag is never passed; wdm72D itself carries no reflectivity path).
!
! Usage: ./wdm7_oracle <case_id>   (1..6 regimes, see build_column)
...
! the fp64 build sets KP=8 via -DWDM7_FP64 + -fdefault-real-8. The JAX port runs
! fp64. PROGNOSTIC parity is to a PREDECLARED physical tolerance, never bitwise
! (see run_wdm7_parity.py).
! =====================================================================
PROGRAM wdm7_oracle
  USE module_mp_wdm7, ONLY : wdm7init, wdm72D, effectRad_wdm7
  IMPLICIT NONE
  ...
  !      as in module_microphysics_driver.F CALL wdm7(...) ----
  ...
  REAL(KP), PARAMETER :: DENS_SNOW=100.0          ! dens (wdm7init arg)
  ...
  ! wdm72D works on t(its:ite,kts:kte); the moisture/number/den/p/delz arrays
  ...
  REAL(KP), DIMENSION(its:ite,kts:kte,4) :: qrs   ! 1=qr, 2=qs, 3=qg, 4=qh
  ...
  REAL(KP), DIMENSION(ims:ime)           :: hail, hailncv
  ...
  REAL(KP), DIMENSION(kts:kte) :: t0, q0, qc0, qr0, qi0, qs0, qg0, qh0
  ...
  ! ---- initialize WDM7 module constants (runs radar_init) ----
  CALL wdm7init(RHOAIR0, RHOWATER, DENS_SNOW, CLIQ, CPV, CCN0, allowed)
  ...
  DO k = kts, kte
    ...
    qr0(k) = qrs(1,k,1); qs0(k) = qrs(1,k,2); qg0(k) = qrs(1,k,3); qh0(k) = qrs(1,k,4)
    nn0(k) = ncr(1,k,1); nc0(k) = ncr(1,k,2); nr0(k) = ncr(1,k,3)
    pii0(k)= pii_col(k)
  END DO

  ! accumulators: rain/snow/graupel/hail are running totals (start at 0); the
  ! ncv + sr are zeroed inside wdm72D.
  rain=0.; rainncv=0.; sr=0.
  snow=0.; snowncv=0.; graupel=0.; graupelncv=0.; hail=0.; hailncv=0.

  ! ---- call the real WDM7 microphysics (wdm72D directly, single column) ----
  CALL wdm72D(t, q, qci, qrs, ncr, den, p, delz                  &
  ...
  CALL effectRad_wdm7(t1d, qc1d, nc1d, qi1d, qs1d, den1d,    &
                      ...
                      kts, kte, 1, 1)
  ! apply the driver's bounds (module_mp_wdm7.F wdm7 wrapper lines ~353-355)
  DO k = kts, kte
  ...
  CALL dump_col3n('NN_OUT', ncr, 1)
  CALL dump_col3n('NC_OUT', ncr, 2)
  CALL dump_col3n('NR_OUT', ncr, 3)
  ...
  WRITE(*,'(A,ES23.15E3)') 'HAIL=',       hail(1)
  WRITE(*,'(A,ES23.15E3)') 'HAILNCV=',    hailncv(1)
...
CONTAINS
  ...
  SUBROUTINE dump_col3(name, arr, m)  ! the 4-component qrs array
    CHARACTER(LEN=*), INTENT(IN) :: name
    REAL(KP), DIMENSION(its:ite,kts:kte,4), INTENT(IN) :: arr
    INTEGER, INTENT(IN) :: m
    INTEGER :: kk
    DO kk = kts, kte
  ...
  SUBROUTINE dump_col3n(name, arr, m)  ! the 3-component ncr array
    CHARACTER(LEN=*), INTENT(IN) :: name
    ...
    DO kk = kts, kte
      WRITE(*,'(A,A,I0,A,ES23.15E3)') name,'[',kk,']=', arr(1,kk,m)
    END DO
  END SUBROUTINE dump_col3n

  SUBROUTINE dump_col2c(name, arr, m)  ! the 2-component qci array
    CHARACTER(LEN=*), INTENT(IN) :: name
  ...
  !  2 = deep mixed-phase with melting layer near surface, ice/snow/graupel/
  !      HAIL aloft -> melting (psmlt/pgmlt/phmlt + numbers), hail accretion,
  !      4-channel sedimentation, hail enhanced-melt pheml/nheml
  !  3 = cold ice/snow column (all subfreezing), ice-supersaturated, some
  !      graupel/HAIL aloft -> ice nucleation/deposition (pigen/pidep/psdep/
  !      pgdep/phdep), psaut/pgaut/phaut
  !  4 = hail/graupel-dominant convective core: large qr/qi/qs/qg/qh + dense
  !      cloud -> riming (psacw/pgacw/phacw/paacw + numbers), piacr/pgacr/psacr/
  !      phacr, pracg(G->H), phacs/phacg, freezing of rain to graupel
  !      (pgfrz/ngfrz), pgwet/phwet wet growth
  !  5 = subsaturated mid-level rain/snow/HAIL falling into dry air
  !      -> evaporation (prevp<0 + Nrevp NR->NCCN), psevp/pgevp/phevp, partial
  !         melt with the melting-layer hail (phmlt + phevp)
  ...
  ! (k=1 lowest model layer). Nc/Nr seeded physically (Nc ~ 3e8/kg in cloud, Nr
  ! ~ 1e5/kg in rain), Nn (CCN) = ccn0. Hail (qh) is seeded in cases 2/3/4/5 so
  ! the hail processes are genuinely exercised, with the column spanning the
  ! freezing level so phmlt/pheml/phevp fire.
  ! ------------------------------------------------------------------
  SUBROUTINE build_column(cid, tt, qq, qcip, qrsp, ncrp, dd, pp, dz, exner, sm)
    ...
    REAL(KP), DIMENSION(its:ite,kts:kte,4), INTENT(OUT) :: qrsp
    ...
    qcip = 0.; qrsp = 0.
    ncrp(:,:,1) = CCN0   ! Nn (CCN background; wdm72D clamps to [1e8,2e10])
    ncrp(:,:,2) = 0.     ! Nc
    ncrp(:,:,3) = 0.     ! Nr
    DO kk = 1, KX
      z_k = zz(kk)
      ...
      CASE (2)   ! mixed-phase: cloud/rain low, ice/snow/graupel/HAIL aloft
        ...
        IF (z_k > 4000.0)  qrsp(1,kk,4) = 3.0E-4*EXP(-((z_k-6000.0)/2500.0)**2)
      CASE (3)   ! cold: ice + snow aloft, some graupel/HAIL
        ...
        IF (z_k > 7000.0) qrsp(1,kk,4) = 8.0E-5*EXP(-((z_k-9000.0)/2500.0)**2)
      CASE (4)   ! hail/graupel convective core: big qr,qi,qs,qg,qh + cloud
        ...
        qrsp(1,kk,4) = 1.5E-3*EXP(-((z_k-6500.0)/2500.0)**2)
      CASE (5)   ! subsaturated: rain + snow + HAIL falling into dry air
        ...
        IF (z_k > 5000.0) qrsp(1,kk,4) = 3.0E-4*EXP(-((z_k-7000.0)/2500.0)**2)
      ...
      ! rain exists (~1e5/kg). Physically plausible double-moment seeds so the
      ! number tendencies are genuinely exercised.
      IF (qcip(1,kk,1) > 1.0E-6) ncrp(1,kk,2) = 3.0E8
      IF (qrsp(1,kk,1) > 1.0E-6) ncrp(1,kk,3) = 1.0E5
    END DO
  END SUBROUTINE build_column

END PROGRAM wdm7_oracle
