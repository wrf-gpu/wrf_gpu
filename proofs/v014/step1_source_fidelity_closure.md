...
- Strict WRF after `conv_t_tendf_to_moist` vs current JAX dry `T_TENDF`: max_abs `438.5379097262689`, rmse `5.4654420375782955`.
- JAX mass-coupled MYNN `RTHBLTEN` remains too weak: max_abs `2449.6284041974827` vs WRF `2522.90576171875`.
- JAX mass-coupled MYNN qv source is also too weak: max_abs `0.5352425747931215` vs WRF `QV_TEND` `0.4930315017700195`.
...
- Forcing radiation only moves max_abs to `442.557323214411`; held `RTHRATEN` is secondary.
