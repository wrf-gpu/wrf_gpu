...
- Strict WRF after `conv_t_tendf_to_moist` vs current JAX dry `T_TENDF`: max_abs `847.1446969755725`, rmse `9.627208432391289`.
- JAX mass-coupled MYNN `RTHBLTEN` remains too weak: max_abs `2299.181282241139` vs WRF `2522.90576171875`.
- JAX mass-coupled MYNN qv source is also too weak: max_abs `0.8961413269865087` vs WRF `QV_TEND` `0.4930315017700195`.
...
- Forcing radiation only moves max_abs to `851.1711251874308`; held `RTHRATEN` is secondary.
