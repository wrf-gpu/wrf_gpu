! =====================================================================
! v0.13 single-column MRF PBL oracle driver (bl_pbl_physics=99).
!
! Drives the UNMODIFIED pristine WRF module_bl_mrf.F:MRF wrapper (which
! calls the internal MRF2D + TRIDI2) on prescribed single-column
! soundings and dumps inputs + WRF MRF tendencies/diagnostics for JAX
...
! v0.13 operational gate uses the fp64 savepoints at ~1e-13.
!
! The six soundings reuse the v0.6.0 YSU oracle regimes so the MRF
! gate spans the same unstable/stable/neutral land + marine regimes.
! =====================================================================
PROGRAM mrf_oracle
  USE module_bl_mrf, ONLY : mrf
  IMPLICIT NONE
  ...
  REAL, DIMENSION(ims:ime,jms:jme) :: PSFC,ZNT,UST,ZOL,HOL,PBL,PSIM,PSIH,XLAND
  REAL, DIMENSION(ims:ime,jms:jme) :: HFX,QFX,TSK,GZ1OZ0,WSPD,BR,REGIME
  ...
  CALL build_case(case_id, name, U, V, T, TH, QV, QC, QI, P, PII, DZ, ZF, &
                  PSFC, ZNT, UST, PSIM, PSIH, XLAND, HFX, QFX, TSK, &
                  GZ1OZ0, WSPD, BR)

  RUBLTEN = 0.0; RVBLTEN = 0.0; RTHBLTEN = 0.0
  RQVBLTEN = 0.0; RQCBLTEN = 0.0; RQIBLTEN = 0.0
  ZOL = 0.0; HOL = 0.0; PBL = 0.0; KPBL2D = 0; REGIME = 0.0
  flag_qi = .FALSE.

  CALL mrf(u3d=U,v3d=V,th3d=TH,t3d=T,qv3d=QV,qc3d=QC,p3d=P,pi3d=PII, &
           ...
           dz8w=DZ,z=ZF,xlv=XLV,rv=R_V,psfc=PSFC, &
           p1000mb=P1000, &
           znt=ZNT,ust=UST,zol=ZOL,hol=HOL,pbl=PBL,psim=PSIM,psih=PSIH, &
           xland=XLAND,hfx=HFX,qfx=QFX,tsk=TSK,gz1oz0=GZ1OZ0,wspd=WSPD,br=BR, &
           dt=60.0,dtmin=1.0,kpbl2d=KPBL2D, &
...
CONTAINS
  ...
  SUBROUTINE build_case(cid, nm, Uu, Vv, Tt, Tht, Qq, Qc, Qi, Pp, Exner, Dz, Zfull, &
    ...
    REAL, DIMENSION(ims:ime,jms:jme), INTENT(OUT) :: Ps,Z0,UstA,PsimA,PsihA,XlandA
  ...
  SUBROUTINE dump_case(cid, nm, Uu, Vv, Tt, Qq, Pp, Exner, Dz, Zfull, &
                       Ps, Z0, UstA, PsimA, PsihA, XlandA, HfxA, QfxA, TskA, &
                       Gz, WspdA, BrA, PblA, KpblA, ZolA, HolA, RegA, &
    ...
    REAL, DIMENSION(ims:ime,jms:jme), INTENT(IN) :: Ps,Z0,UstA,PsimA,PsihA,XlandA
    REAL, DIMENSION(ims:ime,jms:jme), INTENT(IN) :: HfxA,QfxA,TskA,Gz,WspdA,BrA,PblA,ZolA,HolA,RegA
    ...
    WRITE(*,'(A,ES23.15)') 'ZOL=', ZolA(1,1)
    WRITE(*,'(A,ES23.15)') 'HOL=', HolA(1,1)
    WRITE(*,'(A,ES23.15)') 'REGIME_OUT=', RegA(1,1)
    CALL dump_col('U', Uu)
    CALL dump_col('V', Vv)
    CALL dump_col('T', Tt)
    CALL dump_col('QV', Qq)
