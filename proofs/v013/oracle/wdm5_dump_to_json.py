#!/usr/bin/env python3
"""Parse the flat key=value dump from wdm5_oracle into a structured JSON savepoint.

Scalars: CASE, KX, DT, RAIN, RAINNCV, SNOW, SNOWNCV, SR.
Columns (length KX, Fortran index 1..KX -> JSON list index 0..KX-1):
  inputs : T_IN, QV_IN, QC_IN, QR_IN, QI_IN, QS_IN,
           NN_IN, NC_IN, NR_IN, PII, DEN, P, DELZ
  outputs: T_OUT, QV_OUT, QC_OUT, QR_OUT, QI_OUT, QS_OUT,
           NN_OUT, NC_OUT, NR_OUT, RE_CLOUD, RE_ICE, RE_SNOW
"""
import json
import re
import sys
...
def main(infile: str, outfile: str, scheme: str = "WDM5 (mp_physics=14)") -> None:
    scalars: dict = {}
    cols: dict = {}
    ...
    kx = scalars["KX"]
    out = {"scheme": scheme, "scalars": scalars, "columns": {}}
...
if __name__ == "__main__":
    scheme = sys.argv[3] if len(sys.argv) > 3 else "WDM5 (mp_physics=14)"
    main(sys.argv[1], sys.argv[2], scheme)
