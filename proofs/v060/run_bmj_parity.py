#!/usr/bin/env python3
"""Isolated BMJ savepoint parity against pristine-WRF oracle JSON.

This is intentionally a report generator, not a test that hides failed parity.
It predeclares tolerances below, evaluates every committed BMJ savepoint, and
writes ``proofs/v060/bmj_savepoint_parity.json`` with PASS/FAIL evidence.
"""
...
# Predeclared before evaluating the JAX port. These are deliberately tight for
# fp64 isolated-column parity against the text-dumped WRF oracle.
...
def main() -> int:
    ...
    if not case_paths:
        raise SystemExit(f"no BMJ savepoints found in {SAVEPOINT_DIR}")
    ...
    for path in case_paths:
        ...
        case_name = path.stem
        rows = [
            _field_report(name, case_name, candidate[name], reference[name])
        ...
        regime_rows.append(
            {
                "case": case_name,
                "pass": regime_pass,
                "candidate": candidate["REGIME"],
                "reference": reference["REGIME"],
            }
        )
        cases.append(
            {
                "case": case_name,
                "oracle_regime": reference["REGIME"],
                "candidate_regime": candidate["REGIME"],
                "field_pass": all(row["pass"] for row in rows),
                "regime_pass": regime_pass,
                "pass": all(row["pass"] for row in rows) and regime_pass,
            }
        )
    ...
    report = {
        "schema": "wrf_gpu2.proofs.v060.bmj_savepoint_parity.v1",
        "status": status,
        "wrf_faithful": status == "PASS",
        "oracle": {
            "type": "unmodified_pristine_wrf_module_cu_bmj_standalone_call",
            "source_path": str(WRF_BMJ_SOURCE),
            "source_sha256": _sha256(WRF_BMJ_SOURCE),
            "generation_command": "taskset -c 0-3 bash proofs/v060/oracle/bmj_build_and_run.sh",
        ...
        "notes": [
            "PASS requires all field residuals within the predeclared abs+rel tolerances and exact regime classification.",
            "This proof compares JAX BMJ against pristine WRF savepoints only; it is not a JAX-vs-JAX self-compare.",
        ],
    }
    REPORT_PATH.write_text(json.dumps(report, indent=2) + "\n")
    print(
        f"BMJ parity {status}: worst={worst['field']} case={worst['case']} "
