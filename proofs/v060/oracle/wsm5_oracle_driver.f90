! v0.6.0 WSM5 single-column oracle driver.
!
! Calls the UNMODIFIED WRF classic WSM5 scheme in phys/module_mp_wsm5.F on
! prescribed short columns and dumps the full pre/post state for JAX parity.
PROGRAM wsm5_oracle
  ...
  USE module_mp_wsm5, ONLY : wsm5, wsm5init
  IMPLICIT NONE
  ...
  REAL, DIMENSION(ims:ime,kms:kme,jms:jme) :: th, qv, qc, qr, qi, qs
  REAL, DIMENSION(ims:ime,kms:kme,jms:jme) :: den, pii, p, delz
  REAL, DIMENSION(ims:ime,kms:kme,jms:jme) :: re_cloud, re_ice, re_snow, refl
  REAL, DIMENSION(ims:ime,jms:jme) :: rain, rainncv, snow, snowncv, sr
  REAL, DIMENSION(kts:kte) :: t0, qv0, qc0, qr0, qi0, qs0, pii0
  REAL :: dt
  INTEGER :: k, case_id
  ...
  CALL build_column(case_id, th, qv, qc, qr, qi, qs, den, pii, p, delz)
  ...
  re_cloud=RE_QC_BG; re_ice=RE_QI_BG; re_snow=RE_QS_BG; refl=-35.

  CALL wsm5(th=th, q=qv, qc=qc, qr=qr, qi=qi, qs=qs, den=den, pii=pii, p=p, delz=delz, &
       ...
       snowncv=snowncv, sr=sr, refl_10cm=refl, diagflag=.false., do_radar_ref=0, &
       has_reqc=1, has_reqi=1, has_reqs=1, re_cloud=re_cloud, re_ice=re_ice, re_snow=re_snow, &
  ...
  CALL dump_3d('QC_OUT', qc)
  CALL dump_3d('QR_OUT', qr)
  CALL dump_3d('QI_OUT', qi)
  CALL dump_3d('QS_OUT', qs)
...
CONTAINS
  ...
  SUBROUTINE build_column(cid, thh, qq, qcc, qrr, qii, qss, dd, exner, pp, dz)
    INTEGER, INTENT(IN) :: cid
    REAL, DIMENSION(ims:ime,kms:kme,jms:jme), INTENT(OUT) :: thh, qq, qcc, qrr, qii, qss
    ...
    DO kk = 1, KX
      ...
      CASE (2)
        ...
        IF (z_k < 5000.0) qrr(1,kk,1) = 6.0E-4*EXP(-((z_k-1200.0)/1500.0)**2)
        IF (z_k > 4000.0) qii(1,kk,1) = 3.0E-4*EXP(-((z_k-7000.0)/2500.0)**2)
        IF (z_k > 3500.0) qss(1,kk,1) = 8.0E-4*EXP(-((z_k-6000.0)/3000.0)**2)
      CASE (3)
        qii(1,kk,1) = 2.0E-4*EXP(-((z_k-6000.0)/3000.0)**2)
        qss(1,kk,1) = 5.0E-4*EXP(-((z_k-5000.0)/3000.0)**2)
      CASE (4)
        IF (z_k < 5000.0) qcc(1,kk,1) = 1.2E-3*EXP(-((z_k-2000.0)/2000.0)**2)
        IF (z_k < 6000.0) qrr(1,kk,1) = 1.5E-3*EXP(-((z_k-2500.0)/2000.0)**2)
        IF (z_k > 4000.0) qii(1,kk,1) = 4.0E-4*EXP(-((z_k-7500.0)/2500.0)**2)
        qss(1,kk,1) = 1.0E-3*EXP(-((z_k-6000.0)/3000.0)**2)
      CASE (5)
        IF (z_k < 6000.0) qrr(1,kk,1) = 7.0E-4*EXP(-((z_k-3000.0)/2000.0)**2)
        qss(1,kk,1) = 6.0E-4*EXP(-((z_k-5000.0)/2500.0)**2)
      CASE DEFAULT
        IF (z_k < 3000.0) qcc(1,kk,1) = 1.0E-5*EXP(-((z_k-1500.0)/1000.0)**2)
      END SELECT
    END DO
  END SUBROUTINE build_column
END PROGRAM wsm5_oracle
