#!/usr/bin/env python3
"""Parse the flat key=value dump from morrison_oracle into a structured JSON savepoint.

Scalars: CASE, KX, DT, RAINNCV, SNOWNCV, GRAUPELNCV, SR.
Columns (length KX, Fortran index 1..KX -> JSON list index 0..KX-1):
  inputs : TH_IN, QV_IN, QC_IN, QR_IN, QI_IN, QS_IN, QG_IN,
           NI_IN, NS_IN, NR_IN, NG_IN, PII, P, DZ, W
  outputs: TH_OUT, QV_OUT, QC_OUT, QR_OUT, QI_OUT, QS_OUT, QG_OUT,
           NI_OUT, NS_OUT, NR_OUT, NG_OUT
"""
import json
import re
import sys
...
def main(infile: str, outfile: str) -> None:
    ...
    with open(infile) as fh:
        for line in fh:
            line = line.rstrip("\n")
            if line.startswith(("INIT_FATAL", "RUN_FATAL", "FATAL", "WRF_ERROR")):
