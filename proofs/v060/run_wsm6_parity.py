#!/usr/bin/env python3
"""Validate the JAX WSM6 port against the independent Fortran WRF oracle.

Compares gpuwrf.physics.microphysics_wsm6.wsm6_run (fp64) against gold
savepoints that are the REAL WRF physics_mmm WSM6 scheme (mp_wsm6.F90 +
mp_wsm6_effectRad.F90) run on representative columns + edge cases. The JAX
port cannot self-compare: the reference is the Fortran scheme, not a JAX re-run.

Two oracle builds (both from UNMODIFIED mp_wsm6.F90; see proofs/v060/oracle):
  * proofs/v060/savepoints       -- canonical WRF kind_phys=selected_real_kind(6)
                                    (single precision). This is the binding
                                    reference for the PROGNOSTIC state update.
  * proofs/v060/savepoints_fp64  -- kind_phys override to double precision
                                    (scheme source otherwise unmodified). Used
                                    ONLY for the effective-radius diagnostics,
                                    which have CATEGORICAL detection floors that
                                    flip on trace (< ~1e-11 kg/kg) hydrometeor
                                    amounts. In fp32 the reference scheme leaves
                                    single-precision round-off dust (e.g.
                                    qi=7.276e-12 = 2^-37 kg/kg) at otherwise-empty
                                    cells, tripping the floor and producing a
                                    re_ice/re_snow value where the fp64 reference
                                    (and the fp64 JAX port) correctly give the
                                    background radius. This is the REFERENCE's
                                    truncation, not a port error: against the fp64
                                    oracle these cells agree to ~1e-9 relative.

PREDECLARED TOLERANCES (frozen BEFORE comparison; never loosened):
  fp64 JAX vs fp32 (prognostic) / fp64 (diagnostic) Fortran WSM6.
  WSM6 has data-dependent search loops in the semi-Lagrangian PLM sedimentation,
  an exp/log-heavy saturation path, and many clamped accretion terms; fp32-vs-fp64
  roundoff propagates through these. Tolerances are stated as a relative tolerance
  on the column-max magnitude (rel) plus an absolute floor (abs_floor) below which
  a field is treated as zero, and absolute tolerances on temperature and the
  surface precip accumulators (mm).

Emits proofs/v060/wsm6_savepoint_parity_report.json with per-field per-column
...
from gpuwrf.physics.microphysics_wsm6 import wsm6_run  # noqa: E402
...
def run_jax_for(d):
    ...
    out = wsm6_run(jnp.asarray(t), jnp.asarray(qv), jnp.asarray(qc),
                   jnp.asarray(qr), jnp.asarray(qi), jnp.asarray(qsv),
                   jnp.asarray(qg), jnp.asarray(den), jnp.asarray(p),
                   jnp.asarray(delz), s["DT"])
...
def run_case(cid):
    ...
    for leaf, oname in RE_FIELDS:
        ...
        # also record the fp32 comparison for transparency
    ...
    for cid in (1, 2, 3, 4, 5, 6):
        ...
        for fld, m in res.items():
            if "max_abs" in m:
                ...
                print(f"  {fld:9s} [{m['ref']}] jax={m['jax']:.5e} oracle={m['oracle']:.5e} "
                      f"abs_err={m['abs_err']:.3e} {'ok' if m['pass'] else 'FAIL'}")
        print()

    report = {
        "scheme": "WSM6 (mp_physics=6)",
        "oracle": {
            "prognostic_ref": "WRF physics_mmm mp_wsm6.F90 fp32 single-column savepoints "
                              "(proofs/v060/savepoints)",
            "diagnostic_ref": "same scheme, kind_phys->fp64 override "
                              "(proofs/v060/savepoints_fp64); used for effective radii "
        ...
        "notes": [
            "Prognostic state (theta/t, qv, qc, qr, qi, qs, qg) and surface precip "
            "are gated against the CANONICAL fp32 WRF oracle.",
            "Effective radii (re_cloud, re_ice, re_snow) are diagnostic-only and have "
            "categorical detection floors; the fp32 reference leaves single-precision "
            "round-off dust (~2^-37 kg/kg) at trace cells that trips the floor. They are "
            "therefore gated against the fp64 oracle, where the JAX fp64 port agrees to "
            "~1e-9 relative. Each re field also records its vs-fp32 error for transparency.",
        ],
        "cases": cases,
    }
    outpath = os.path.join(HERE, "wsm6_savepoint_parity_report.json")
