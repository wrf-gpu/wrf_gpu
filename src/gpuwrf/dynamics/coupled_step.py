"""Validation-only M6B6 coupled timestep composition.

This module is the final B-direct parity rung: M6B5 dycore-step snapshots with
the already-ported M5 physics callables and Gen2 lateral-boundary replay turned
on. It is intentionally not imported by operational runtime.
...
from gpuwrf.dynamics.acoustic_loop import AcousticLoopState, FULL_STATE_FIELDS
...
class CoupledStepConfig:
    """Static validation config for M6B6 coupled-step composition."""
    ...
    def dycore_config(self) -> DycoreStepConfig:
...
def _adaptive_boundary_config(acoustic: dict[str, Any], cfg: CoupledStepConfig) -> BoundaryConfig:
...
def _physics_update(acoustic: dict[str, Any], extras: dict[str, Any], cfg: CoupledStepConfig) -> tuple[dict[str, jax.Array], dict[str, jax.Array]]:
...
def _boundary_update(acoustic: dict[str, Any], extras: dict[str, Any], cfg: CoupledStepConfig, step_index: int) -> tuple[dict[str, jax.Array], dict[str, jax.Array]]:
    state_before = _state_from_acoustic(acoustic, extras, cfg, physical_theta=False)
    lead_seconds = jnp.asarray(max(int(step_index) - 1, 0), dtype=jnp.float64) * float(cfg.dt)
...
def coupled_timestep_wrf(
    ...
    *,
    ...
    """Run one validation-only WRF-ordered coupled timestep."""

    if int(cfg.rk_order) != 3:
        raise ValueError("M6B6 coupled_step validation requires RK3")
    if not bool(cfg.physics_enabled) or not bool(cfg.boundary_enabled):
        raise ValueError("M6B6 coupled_step validation requires physics and boundary enabled")
    extra = dict(extras or {})
    _rk, dycore = dycore_timestep_wrf(state, metrics, cfg.dycore_config())
...
def coupled_timesteps_wrf(
    ...
    *,
    ...
    """Run repeated validation-only coupled timesteps."""

    current = state
    snapshots: list[dict[str, jax.Array]] = []
    for step in range(1, int(steps) + 1):
        snapshot = coupled_timestep_wrf(current, metrics, cfg, extras=extras, step_index=step)
