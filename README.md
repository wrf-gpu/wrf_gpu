...
**v0.13.0 is "Validate & Accelerate": it lifts the single-GPU VRAM ceiling that gated v0.12.0, turns gravity-wave drag on by default on the nested 1 km path, and hardens the validation/reproducibility story.** The keystone is a three-part RRTMG VRAM reduction — g-point band-tiling + taumol/optics construction chunking + leading-column tiling — that cut peak shortwave VRAM **−88.6 %** and longwave **−43.6 %** at production column depth, then capped the formerly full-column g-point transient (`proofs/v013/rrtmg_column_tile_vram_suite.json`: LW untiled OOM on 32.11 GiB allocation, LW tiled 5.37 GiB; SW 10.03 → 1.62 GiB). The transformations are numerically inert (`max_rel = 0.0`). That headroom unblocked the v0.12.0-deferred **24 h nested 1 km + GWD** run, which now passes `PIPELINE_GREEN`, so gravity-wave drag (`gwd_opt=1`) is **default-on on the nested path** (it OOM'd at ~sim-hr 7 in v0.12.0). v0.13.0 also re-lands the compile-speed infra (AOT precompile + persistent XLA autotune cache, GPU-validated — the v0.12.0 GPU-abort fixed), wires **MYJ PBL + Janjic-Eta sfclay** to operational, adds **clear-sky radiation diagnostics** and **multi-GPU `shard_map` fake-mesh sharding** (bit-identical; real throughput HW-deferred), wires **moisture flux-advection into the RK3 large step** (correctness, opt-in), passes an independent **RRTM-LW skeptic audit** (+2 fixes), and ships **outsider-runnable reproducibility** + a **community-standard validation suite**.
