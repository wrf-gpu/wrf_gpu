...
The validation lane produces the binding skill + speedup numbers below, filled from the v0.9.0 validation burst (single representative MAM case; honest about the open d03 1 km gate and the unscored n=15 TOST).
...
- **Coupled vs CPU-WRF, d02 (3 km).** Combined-physics GPU forecast (gated-fp32 ship mode, radiation-ON) vs 28-rank CPU-WRF v4.7.1 `wrfout`, 72 h, one representative MAM case (`20260507_18z`). **Finite and stable all 72 h, no blow-up** (proof [`proofs/v090/d02_coupled_skill_72h.json`](proofs/v090/d02_coupled_skill_72h.json), run via the d02-replay harness with the validated stability namelist). Per-lead RMSE vs CPU-WRF truth: **T2 within bar (3.0 K) at every one of the 72 leads** (mean 1.06 K, max 1.42 K, final-hour 0.81 K); **V10 within bar (7.5 m/s) at every lead** (mean 3.21 m/s, final 2.97 m/s); **U10 within bar at 66/72 leads** (mean 4.79 m/s, final 4.00 m/s) — it transiently breaches the 7.5 m/s bar (max 8.04 m/s) during the diurnal peak-wind window (~h22–30) then recovers, an *episodic near-surface westerly under-prediction*, not a degrading instability. HFX/PBLH stay inside their informational bands (HFX mean 60 / final 45 W m⁻²; PBLH mean 182 / final 265 m). **Honest verdict:** the proof's machine `status` is `FAIL` *solely* because the all-leads-within-bar predicate trips on those 6 U10 leads; the final-hour Tier-4 RMSE passes on all of T2/U10/V10 and T2/V10 pass at every lead. This is the **operational equivalence evidence** (single case, single season), not a green-on-every-lead claim. Two honesty caveats: (a) the production daily-pipeline path (hourly land-state refresh) on the only other data-available d02 config (`20260521`) is itself unstable in gated-fp32 (NaNs ~h1, qke canary — same structural steep-terrain signature as d03 below), so this 72 h skill rests on the replay-harness path; (b) `precip` is not separately scored here.
- **Coupled vs CPU-WRF, d03 (1 km).** Mandatory 1 km gate (every output timestep within scientifically-acceptable margins) with the v0.9.0 faithful physics. **OPEN ISSUE / CARRIED OVER — the 1 km gate is NOT met in the gated-fp32 ship mode.** The d03 Tenerife forecast goes **non-finite after forecast hour 1** (qke the sole offending field, ~3036 cells over ~69 steep-terrain columns); because no timesteps complete, no T2/U10/V10/PBLH/precip RMSE could be scored. The original "qke fp32 overflow" hypothesis was **falsified**: promoting qke to fp64 (now shipped) reproduces the *identical* blow-up at tiny qke magnitudes, so this is a **dynamics-driven structural instability over steep terrain, not a precision-range problem**. **1 km is finite in full fp64** over the confirmed 0.3 h / 360-step window, but fp64 is ~1:64-throttled on the RTX 5090 so a full 24 h fp64 validation is impractically slow. 3 km d02 (above) IS stable and validated in gated-fp32 and is the primary operational resolution. Full write-up + carry-over: [`docs/KNOWN_ISSUES.md`](docs/KNOWN_ISSUES.md), proofs [`proofs/v090/d03_1km_validation.json`](proofs/v090/d03_1km_validation.json) + [`proofs/v090/d03_1km_validation_qkefix.json`](proofs/v090/d03_1km_validation_qkefix.json). Tracked for the post-0.9.0 numerics/stability sprint. *(The CPU 1 km reference is also indicative only — it ran inside a contended 5-domain nest, not a clean standalone.)*
- **Powered TOST equivalence (n=15).** Statistical equivalence of 24–72 h RMSE on **T2 / U10 / V10** under TOST at the ADR-029 predeclared margins (10% of the local CPU-WRF benchmark RMSE: **T2 ±0.215 K, U10 ±0.231 m/s, V10 ±0.275 m/s**). **n=15 is the binding floor and is honestly underpowered** relative to the ADR-029 target (n≈27 to detect a 10% RMSE difference at α=0.05, β=0.20). The result is labeled single-season (MAM) and underpowered, never an unqualified "equivalence PASS." Margins + power analysis: [`.agent/decisions/ADR-029-STATISTICS-DESIGN-TOST.md`](.agent/decisions/ADR-029-STATISTICS-DESIGN-TOST.md). **TOST result:** the n=15 CPU-WRF MAM corpus is **prepared** (forcing retained, CPU-WRF reference runs assembled/backfilled), but the **formal n=15 TOST has not yet been scored for v0.9.0** — it is the powered analysis carried by the paper, not a v0.9.0 ship gate. The **operational equivalence evidence for v0.9.0 is the d02 (3 km) coupled-skill result above** (final-hour Tier-4 RMSE within bars; T2/V10 within bar at every lead). **No "TOST PASS" / "equivalence PASS" is claimed here** — doing so on an unscored n=15 would be an over-claim.
- **End-to-end wall-clock speedup.** Honest command-to-finish wall-clock (CPU wall-clock ÷ GPU wall-clock), single RTX 5090 vs 28-rank CPU-WRF v4.7.1 on the same workstation, compile-inclusive headline, gated-fp32 ship mode (proof [`proofs/v090/speedup_benchmark.json`](proofs/v090/speedup_benchmark.json)).
  - **9/3 km nested (d02), real-user-time, WARM cache (operational daily cadence), 72 h — headline ≈ 2.16× (conservative) / 2.41× (mid) / 2.59× (high).** This is the honest command-to-finish ratio against the d02 own-solver CPU cost, compile-inclusive, with the JAX persistent compile cache warm (the realistic daily-run scenario).
  - **9/3 km nested (d02), COLD first-ever launch, 24 h — ≈ 1.33×** (pays the one-time ~7-min RRTMG/physics/dycore XLA compile; amortizes away on subsequent runs via the persistent cache).
  - **1 km (d03) speedup — UNMEASURED / BLOCKED**, because the gated-fp32 d03 run NaNs at hour 1 (the open issue above) so no complete command-to-finish wall exists at ship precision; full-fp64 is too throttled to time a 24 h run.
  - **Kept clearly separate (NOT the headline):** the prior published **kernel / compute-only (compile-EXCLUDED) ceiling of ≈ 5.3×–7.84×** ([`publish/runtime_optimization_analysis.md`](publish/runtime_optimization_analysis.md)) is a *steady-state per-step* number, not real-user wall-clock; do not conflate it with the 2.16× end-to-end headline. The compile-excluded *real-user* steady-state for this benchmark is ≈ 2.33×–2.79× (context only); the strict dt-matched floor (GPU forced to the CPU 6 s step) is ≈ 1.29× warm.
