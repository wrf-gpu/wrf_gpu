...
Status (2026-06-10 06:31 WEST): **v0.14 is the active closure target. Grid-cell
...
|---|---|---|
| Step-1/grid-cell parity | Active. MYNN cold-start QKE, MYNN first-call surface semantics, WRF `LANDUSE.TBL`-backed `TSK/ZNT/MAVAIL`, WRF `phy_prep` thermodynamic inputs, local `SFCLAY1D_mynn` output algebra, MYNN dry-theta/phy_prep source coupling, and WRF surface/land flux handoff localization are fixed/bounded at WRF internal hooks. The latest proof (`proofs/v014/step1_surface_land_flux_handoff.*`) shows WRF `SFCLAY1D_mynn` output equals `PRE_NOAHMP`, NoahMP is the exact HFX/QFX change point, and `POST_NOAHMP` equals MYNN driver input. Strict after-conv `T_TENDF` remains red at max_abs `438.5379097262689`, RMSE `5.4654420375782955`. The active blocker is now JAX Step-1 built with NoahMP disabled/missing land/static state: `use_noahmp=False`, `sf_surface_physics=None`, `inputs_have_noahmp_land=False`. | Fable/Mythos whole-task sprint is staged at `.agent/sprints/2026-06-10-v014-fable-noahmp-step1-closure/`: wire WRF-derived NoahMP land/static/radiation state and `sf_surface_physics=4` semantics into the Step-1 live-nest/source-capture path, fix adjacent same-path bugs if exposed, then rerun `proofs/v014/step1_mynn_source_coupling.py` and strict Step-1. No TOST/Switzerland/FP32 until fixed or bounded. |
