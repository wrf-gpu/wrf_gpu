...
Status (2026-06-10 04:26 WEST): **v0.14 is the active closure target. Grid-cell
...
|---|---|---|
| Step-1/grid-cell parity | Active. MYNN cold-start QKE, MYNN first-call surface semantics, WRF `LANDUSE.TBL`-backed `TSK/ZNT/MAVAIL`, and WRF `phy_prep` thermodynamic inputs are fixed/proven at the `sfclay_mynn` hook. Fixed input maxima: `th_phy(kts)` `6.71089752017906e-05 K`, `t_phy(kts)` `0.013577942721781255 K`, hydrostatic `p_phy(kts)` `0.015625 Pa`, `dz8w(kts)` `0.00018988715282830526 m`, `psfc` `0.015625 Pa`. Strict after-conv residual improved but remains red: max_abs `847.1445725702908`, RMSE `9.56593990212596`. Current narrow blocker is later `module_sf_mynn` surface-layer output algebra after the fixed input tuple: `UST` max_abs `0.01231782267117762`, `HFX` max_abs `27.09163832864155`, `QFX` max_abs `2.744275103194571e-07`, `BR` max_abs `2.0`. | Open a GPT-5.5 xhigh surface-layer-output sprint: add a narrow WRF internal hook inside `module_sf_mynn.F` / `SFCLAY1D_mynn` for `thx/thgb/br/zol/psim/psih/ust/hfx/qfx`, compare against `surface_layer_with_diagnostics` on the fixed input tuple, fix local algebra if proven, then rerun strict Step-1. No TOST/Switzerland until fixed or bounded. |
