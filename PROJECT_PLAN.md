...
Status (2026-06-10 11:00 WEST): **v0.14 is the active closure target. Grid-cell
...
|---|---|---|
| Step-1/grid-cell parity | Active. MYNN cold-start QKE, MYNN first-call surface semantics, WRF `LANDUSE.TBL`-backed `TSK/ZNT/MAVAIL`, WRF `phy_prep` thermodynamic inputs, local `SFCLAY1D_mynn` output algebra, MYNN dry-theta/phy_prep source coupling, WRF surface/land flux handoff localization, the JAX Step-1 NoahMP-disabled configuration gap, NoahMP land-tile energy forcing, and the NoahMP/sfclay water-path moist-theta bug are fixed/bounded at WRF internal hooks. Fable/Mythos strict-closure sprint is accepted as a major narrowing: `proofs/v014/surface_layer_theta_decoupling.*` proves the production `noahmp_surface_hook._build_column_view(..., grid=...)` fix supplies WRF `phy_prep` dry `t_air`, true `psfc`, hydrostatic `p`, and density to the revised surface layer; water HFX RMSE improves `11.87 -> 1.37 -> 0.0118 W/m2` and water `ust` is near exact. Strict after-conv `T_TENDF` improves from max_abs `1489.5135568470864`, RMSE `12.146876720723487` to max_abs `53.52301833555157`, RMSE `2.5444971494115354`; it is still red. The remaining dominant blocker is now MYNN-EDMF `RTHBLTEN` in `module_bl_mynnedmf` (land and water, ~4-7% local tendency where large). RRTMG `RTHRATEN`/GLW/SWDOWN remains real but secondary (`RTHRATEN` max about `19.4`) and is localized by `proofs/v014/rrtmg_step1_forcing_parity.*`. | Active sprint: `.agent/sprints/2026-06-10-v014-fable-mynn-rthblten-closure/`. Endpoint: strict Step-1 green or a formal WRF-anchored MYNN-EDMF `RTHBLTEN` bound; only after that close/bound secondary RRTMG forcing. No TOST/Switzerland-GPU/FP32 until strict Step-1 is fixed or formally bounded. |
