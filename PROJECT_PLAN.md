...
Status (2026-06-10 13:45 WEST): **v0.14 is the active closure target. The
...
| Switzerland/Gotthard | CPU24 proof/case exists, but it is not resumable to 72h: no `wrfrst_d0*`, `restart=.false.`, and `wrfbdy_d01` covers only 0-21h. The first 72h gate uses the 129x129/128-mass-point grid to match the accepted CPU24 baseline and avoid first-pass OOM risk. | Build/run a fresh 72h CPU-WRF truth case with resource CSVs, then run matched 72h GPU-JAX and Grid-Delta Atlas. |
| Canary field parity | L2 d02 retained CPU-WRF truth is already complete for 15 x 72h cases in `/mnt/data/canairy_meteo/runs/wrf_l2_backfill_output`. d03 retained truth is mainly 24h, so d02 is the mandatory 72h v0.14 gate; d03/1 km follows as secondary evidence after the two 72h gates are green. | Finish the current short d02 field falsifier, then run one selected L2 d02 72h GPU case against retained CPU truth with resource CSVs and Grid-Delta Atlas. |
