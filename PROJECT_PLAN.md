...
Status (2026-06-10 08:40 WEST): **v0.14 is the active closure target. Grid-cell
...
|---|---|---|
| Step-1/grid-cell parity | Active. MYNN cold-start QKE, MYNN first-call surface semantics, WRF `LANDUSE.TBL`-backed `TSK/ZNT/MAVAIL`, WRF `phy_prep` thermodynamic inputs, local `SFCLAY1D_mynn` output algebra, MYNN dry-theta/phy_prep source coupling, WRF surface/land flux handoff localization, and the JAX Step-1 NoahMP-disabled configuration gap are fixed/bounded at WRF internal hooks. The latest committed proof (`proofs/v014/noahmp_step1_closure.*`) reports `use_noahmp=True`, `sf_surface_physics=4`, and WRF-derived NoahMP land/static/radiation state in the Step-1 path. Strict after-conv `T_TENDF` remains red against pinned one-run WRF truth at max_abs `1489.5135568470864`, RMSE `13.2001844004901`. Active Fable/Mythos lane: NoahMP Step-1 forcing/energy closure; latest live finding is that exact-WRF inputs make the JAX NoahMP energy solve match WRF, while the real Step-1 forcing has large `sfctmp` bias (`~+4 K`) and `LWDN/GLW` bias (`~+14.7 W/m2`), pointing at θ_m/dry-θ forcing semantics plus secondary radiation forcing. New audit `proofs/v014/moist_theta_physics_consumer_audit.*` broadens this: multiple runtime physics consumers need dry conversion at adapter input and moist recoupling on writeback. Secondary RRTMG GLW/RTHRATEN residual is localized, not closed: `proofs/v014/rrtmg_step1_forcing_parity.*` names the remaining boundary as clear-sky derived RRTMG optical/gas/top-buffer profile or downstream kernel boundary. | Let Fable/Mythos finish the NoahMP forcing/energy source/proof fix. Then close the broader moist-theta adapter boundary for the selected v0.14 validation path, starting from shared dry/moist helpers and the highest-impact adapters named by the audit. Close RRTMG with a temporary WRF forcing hook or record a formal demotion. No TOST/Switzerland-GPU/FP32 until strict Step-1 plus the relevant θ_m adapter boundary are fixed or explicitly bounded. |
...
| Switzerland/Gotthard | CPU truth/cases exist. A 24-rank CPU comparison rerun with resource logging is complete: `/mnt/data/wrf_gpu_validation/v014_switzerland_cpu24_20260610T073414Z`, 25 `wrfout`, `rc=0`, `SUCCESS COMPLETE WRF`, last-frame finite PASS, total wall `1084.6 s`, mainloop `1078.4 s`, peak sampled total WRF-rank RSS `12563.766 MiB`. No post-v0.14-fix GPU-vs-CPU proof yet. | Run the GPU side only as v0.14 validation gate after parity/memory stabilization, using the CPU24 run as matched timing/memory baseline. |
