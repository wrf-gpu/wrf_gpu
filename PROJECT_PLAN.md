...
Status (2026-06-10 09:30 WEST): **v0.14 is the active closure target. Grid-cell
...
|---|---|---|
| Step-1/grid-cell parity | Active. MYNN cold-start QKE, MYNN first-call surface semantics, WRF `LANDUSE.TBL`-backed `TSK/ZNT/MAVAIL`, WRF `phy_prep` thermodynamic inputs, local `SFCLAY1D_mynn` output algebra, MYNN dry-theta/phy_prep source coupling, WRF surface/land flux handoff localization, and the JAX Step-1 NoahMP-disabled configuration gap are fixed/bounded at WRF internal hooks. Fable/Mythos closed the NoahMP land-tile energy lane: `proofs/v014/noahmp_land_tile_energy_closure.*` proves the JAX NoahMP energy solve matches WRF exact inputs (FSH RMSE `7.7e-4 W/m2`) and fixes the production `sfctmp` bug by decoupling WRF moist theta `theta_m` to dry theta before Exner conversion. Strict after-conv `T_TENDF` still remains red in `proofs/v014/noahmp_step1_closure.*` at max_abs `1489.5135568470864`, RMSE `12.146876720723487`; the worst cell is a water column, so NoahMP is no longer the limiter. New audit `proofs/v014/moist_theta_physics_consumer_audit.*` broadens this: multiple runtime physics consumers need dry conversion at adapter input and moist recoupling on writeback. Secondary RRTMG GLW/RTHRATEN residual is localized, not closed: `proofs/v014/rrtmg_step1_forcing_parity.*` names the remaining boundary as clear-sky derived RRTMG optical/gas/top-buffer profile or downstream kernel boundary. | Close the same `theta_m -> theta_dry` boundary in the surface-layer/sfclay/MYNN water path with MYNN re-validation, then close RRTMG Step-1 forcing parity with a temporary WRF forcing hook or record a formal demotion. No TOST/Switzerland-GPU/FP32 until strict Step-1 plus the relevant θ_m adapter boundary are fixed or explicitly bounded. |
