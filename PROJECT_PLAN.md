...
Status (2026-06-10 03:39 WEST): **v0.14 is the active closure target. Grid-cell
...
|---|---|---|
| Step-1/grid-cell parity | Active. MYNN cold-start QKE, MYNN first-call surface semantics, and WRF `LANDUSE.TBL`-backed `TSK/ZNT/MAVAIL` sourcing are fixed/proven at the `sfclay_mynn` hook. `TSK` max_abs is `0.0 K`; `ZNT` max_abs is `1.1920928910669204e-08 m`; `MAVAIL` max_abs is `1.1920928966180355e-08`. Strict after-conv residual remains red: max_abs `1497.6112467075195`, RMSE `13.252694871222973`. Current narrow blocker is non-surface thermodynamic column input before `sfclay_mynn`: `th_phy(kts)` max_abs `5.490148027499686 K`, derived `t_phy(kts)` max_abs `5.521345498302992 K`, `p_phy(kts)` max_abs `292.8203125 Pa`; `u/v/qv` are near roundoff. | Open a GPT-5.5 xhigh thermodynamic-column sprint: localize `th_phy(kts)`, `t_phy(kts)`, `p_phy(kts)`, and `dz8w` against JAX `_surface_column_view`, fix Step-1 temperature/pressure sourcing if local, then rerun strict Step-1. No TOST/Switzerland until fixed or bounded. |
