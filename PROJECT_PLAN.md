...
|---|---|---|
| Step-1/grid-cell parity | Active. Latest committed proof will be the Step-1 tendency split: stale RK1 `P_STATE` remains closed, and the live boundary is WRF `first_rk_step_part2` theta source-leaf construction. Full-domain `T_TENDF` at `after_first_rk_step_part2` differs from JAX dry source by max_abs `2457.5830078125`; `rad_rk_tendf=1` is falsified. | Open the internal `first_rk_step_part2` split: truth after `calculate_phy_tend`, `update_phy_ten`, and `conv_t_tendf_to_moist`, with raw `RTH*TEN` / `T_HIST_SRC` contributors. No TOST/Switzerland until this boundary is fixed or bounded. |
