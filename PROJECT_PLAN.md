...
|---|---|---|
| Step-1/grid-cell parity | Active. MYNN cold-start QKE is fixed and MYNN first-call surface semantics are now ported (`UST` first guess, `MOL=0`, land `QSFC=qv/(1+qv)`, Li_etal_2010 z/L seed). Surface boundary metrics improved (`UST` RMSE `0.0867 -> 0.0295`, qv-flux RMSE `1.98e-5 -> 1.44e-5`), but strict after-conv residual remains red: max_abs `1497.6112467075195`, RMSE `13.296448784742802`. Current narrow blocker is WRF-anchored `TSK/ZNT` surface input sourcing before `sfclay_mynn` (`TSK` max_abs `8.344940187890643 K`, `ZNT` max_abs `0.9737602076530456 m`). | Open a GPT-5.5 xhigh TSK/ZNT sprint: emit a tiny WRF Step-1 hook around `module_surface_driver/module_sf_mynn` for incoming `TSK/ZNT/UST/QSFC/MOL` and outgoing `UST/HFX/QFX/ZNT`, compare exact arrays against JAX `_surface_column_view` inputs/diagnostics, fix if local, then rerun strict Step-1. No TOST/Switzerland until fixed or bounded. |
