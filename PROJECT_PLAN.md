...
Status (2026-06-10 13:15 WEST): **v0.14 is the active closure target. Grid-cell
parity gates the release; proof-backed v0.14 memory fixes are merged; long
field-parity/stability validation is now the primary final validation path, with
TOST retained as a secondary station sanity gate.**

Current release-manager state:

| Lane | Status | Next manager action |
|---|---|---|
| Step-1/grid-cell parity | RRTMG dry-temperature input bug fixed and bounded in the active closeout sprint. Latest proofs: `proofs/v014/rrtmg_rthraten_closure.*`, `proofs/v014/rrtmg_step1_forcing_parity.*`, `proofs/v014/mynn_rthblten_step1_closure.*`, and `proofs/v014/noahmp_step1_closure.*`. The exact pre-fix boundary was `RRTMG_LWRAD:T3D=t`; metric-backed `_rrtmg_column_inputs` now decouples `theta_m` to dry theta before RRTMG temperature conversion. GLW RMSE improves `17.5203 -> 0.3515 W/m2`; mass-coupled RTHRATEN RMSE improves `2.4884 -> 0.3646`, max_abs `19.4253 -> 2.7984`. Strict Step-1 remains formally red/bounded at max_abs `55.9297`, RMSE `0.4997`, p99 `0.9529`; MYNN owns the worst-cell max/floor, while remaining RRTMG explains `63.5%` of WRF-QKE RMSE variance. | Commit the RRTMG sprint, then record a tolerance-policy decision for the non-bitwise MYNN/RRTMG mass-coupled Step-1 gate. Next validation signal should be a short operational all-field rollout falsifier; if clean, run long field-parity/stability validation before station-only TOST. |
...
| Grid-Delta Atlas + TOST | Required final evidence hierarchy: all-cell/all-field Grid-Delta Atlas plus long field-stability run is primary; ADR-029 station TOST is secondary sanity evidence. Atlas tooling is merged (`07e1ab2e`) and ready for post-parity validation data. | Run a short field falsifier first, then a 72h/120h Canary or Switzerland CPU-vs-GPU field-parity/stability campaign with compact plots. TOST resumes only after that branch is stable or in parallel as secondary evidence. |
