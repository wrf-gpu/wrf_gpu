...
| Switzerland/Gotthard | CPU truth/cases exist. A 24-rank CPU comparison rerun with resource logging is running at `/mnt/data/wrf_gpu_validation/v014_switzerland_cpu24_20260610T073414Z` so the later GPU-vs-CPU report has matching CPU memory/timing evidence. No post-v0.14-fix GPU-vs-CPU proof yet. | Let the CPU rerun finish and archive timing/memory summary. Run the GPU side only as v0.14 validation gate after parity/memory stabilization. |
