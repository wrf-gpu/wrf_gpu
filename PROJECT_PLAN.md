...
Status (2026-06-10 05:58 WEST): **v0.14 is the active closure target. Grid-cell
...
|---|---|---|
| Step-1/grid-cell parity | Active. MYNN cold-start QKE, MYNN first-call surface semantics, WRF `LANDUSE.TBL`-backed `TSK/ZNT/MAVAIL`, WRF `phy_prep` thermodynamic inputs, local `SFCLAY1D_mynn` output algebra, and MYNN dry-theta/phy_prep source coupling are fixed/bounded at WRF internal hooks. The MYNN source-coupling sprint narrowed the remaining blocker upstream of MYNN: WRF changes heat/moisture fluxes between `SFCLAY1D_mynn` output and the MYNN driver input. Strict after-conv `T_TENDF` remains red at max_abs `438.5379097262689`, RMSE `5.4654420375782955`; WRF-input MYNN raw `RTHBLTEN` is near faithful (max_abs `0.00026206000797283305`, RMSE `2.5971191677632803e-06`, corr `0.9999580118448544`). Handoff deltas: UST max_abs `4.998779168374767e-12`, HFX `277.80298614281253`, QFX `1.4684322196e-05`. | Surface/land flux handoff sprint is opened at `.agent/sprints/2026-06-10-v014-step1-surface-land-flux-handoff/`. GPT-5.5 xhigh should hook WRF immediately before/after `module_surface_driver` `sf_surface_physics=4` flux updates (`HFX/QFX/LH/TSK/GRDFLX` and available diagnostic CH fields), compare to the JAX Step-1 path, wire/fix the land flux overlay into the MYNN bottom-boundary handles if local, then rerun `proofs/v014/step1_mynn_source_coupling.py` and strict Step-1. No TOST/Switzerland/FP32 until fixed or bounded. |
