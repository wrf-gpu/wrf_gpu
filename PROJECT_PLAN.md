...
Status (2026-06-10 22:49 WEST): **v0.14 is the active closure target. The
release gate is now two 72h CPU-WRF vs GPU-JAX field-parity/stability campaigns
(Switzerland/Gotthard and Canary L2 d02) plus Grid-Delta Atlas plots. Powered
TOST is secondary station sanity evidence only, not a tag gate. The d01 Noah-MP
LU16/sand nonfinite blocker is root-caused, fixed, merged, and GPU-confirmed
green. The post-fix Canary h1-h4 Noah-MP land gate is now accepted, and the
full Canary L2 d02 72h GPU field-parity/stability run is in flight.**

Operational note: reports rooted in `/home/enric/src/canairy_waves` are from a
different project and are not part of this v0.14 roadmap.

Current release-manager state:

| Lane | Status | Next manager action |
|---|---|---|
| Step-1/grid-cell parity | RRTMG dry-temperature input bug fixed and bounded in `7fc834c4`. Latest proofs: `proofs/v014/rrtmg_rthraten_closure.*`, `proofs/v014/rrtmg_step1_forcing_parity.*`, `proofs/v014/mynn_rthblten_step1_closure.*`, and `proofs/v014/noahmp_step1_closure.*`. The exact pre-fix boundary was `RRTMG_LWRAD:T3D=t`; metric-backed `_rrtmg_column_inputs` now decouples `theta_m` to dry theta before RRTMG temperature conversion. GLW RMSE improves `17.5203 -> 0.3515 W/m2`; mass-coupled RTHRATEN RMSE improves `2.4884 -> 0.3646`, max_abs `19.4253 -> 2.7984`. Strict Step-1 remains formally red/bounded at max_abs `55.9297`, RMSE `0.4997`, p99 `0.9529`; MYNN owns the worst-cell max/floor, while remaining RRTMG explains `63.5%` of WRF-QKE RMSE variance. | Apply `.agent/decisions/V0140-STEP1-TOLERANCE-POLICY.md`: old strict Step-1 tolerance is diagnostic, not release-green. Next validation signal should be a short operational all-field rollout falsifier; if clean, run long field-parity/stability validation before station-only TOST. |
| Memory/FP32 | Memory lane accepted and merged: MYNN BouLac leading-column tiling (`26815feb`) cuts the measured whole-batch MYNN temp from 14.7 GiB to 3.2 GiB on the 641x321x50 target; shared RK-stage transport velocities landed as exact hygiene. The latest post-LU16-fix exact-branch preflight is green: `/mnt/data/wrf_gpu_validation/v014_noahmp_l2_preflight_fix_20260610T205333Z`, `rc=0`, verdict `PASS_SHORT_GPU_PREFLIGHT` / `PIPELINE_GREEN`, all domains finite, peak total VRAM `9783 MiB`, no OOM markers. FP32 R0 default-inert acoustic precision-mode contract landed (`bc847db2`), but R1/R2 mixed precision remains blocked until the fp64 validation frontier is closed. | Do not spend Fable/Mythos on memory now. FP32 R1 is v0.15 unless field-gate failure forces a v0.14 revisit. |
| GPU validation hygiene | Runbook and wrappers exist (`docs/GPU_RUNBOOK.md`, `scripts/run_gpu_lowprio.sh`, `scripts/run_powered_tost_n15.sh`). Commit `66cc65fa` adds CSV resource logging for GPU/process/host-memory streams. | Keep GPU jobs serialized through the lock wrapper; long validation starts only after grid and memory branches stabilize. |
| Switzerland/Gotthard | CPU72 truth is complete at `/mnt/data/wrf_gpu_validation/v014_switzerland_72h_cpu_20260610T122909Z/run_cpu`: 73 frames, `rc=0`, finite final frame, 24 dmpar ranks, resource CSVs captured. | Launch Switzerland/Gotthard 72h GPU after the Canary 72h GPU run releases the GPU lock and its first-pass result is green/bounded. |
| Canary field parity | L2 d02 retained CPU-WRF truth is complete. The pre-fix GPU 72h run at `/mnt/data/wrf_gpu_validation/v014_canary_d02_72h_moistcqw_20260610T171818Z` completed as a frozen-land stability baseline, not release-green evidence. h24/h36/h48 show no renewed LBC/dycore runaway, but Fable proved the missing-physics wiring bug: standalone nested pipeline never enabled Noah-MP, so land `TSK` was frozen. Source fix is merged/pushed: `c2310c5b` activates/seeds Noah-MP per nested domain when `sf_surface_physics=4`; `c6800bfa` adds the offline h1-h4 land-gate scorer. The subsequent d01 LU16 nonfinite blocker is closed: `22a2cc0c` fixes Noah-MP WATER 1-based soil/veg gathers; `aff7d124`/`5a708074` record proof + GPU confirmation. Root cause was ISLTYP=1 sand reading the all-zero dummy row (`SMCMAX=0`) because WATER used `category - 1`; all other soil categories were also one hydraulic row off. GPU confirmation root `/mnt/data/wrf_gpu_validation/v014_noahmp_l2_preflight_fix_20260610T205333Z` is `rc=0`, `PIPELINE_GREEN`, all domains finite. Post-fix h1-h4 land gate is accepted: `/mnt/data/wrf_gpu_validation/v014_canary_d02_noahmp_lu16fix_h4_20260610T212056Z`, `NOAHMP_NESTED_GPU_H4_ACCEPT`, peak GPU memory `20975 MiB`; h2-h4 land TSK/HFX biases are inside the predeclared thresholds. The full 72h GPU run is in flight at `/mnt/data/wrf_gpu_validation/v014_canary_d02_72h_noahmp_lu16fix_20260610T214731Z`, with h24 intermediate compare armed. | Monitor Canary 72h, inspect h24 intermediate compare when it appears, then full h72 compare. If green/bounded, launch Switzerland/Gotthard 72h GPU. |
| Grid-Delta Atlas + TOST | Required final evidence hierarchy: all-cell/all-field Grid-Delta Atlas plus the two mandatory 72h field-stability runs. ADR-029 station TOST is secondary sanity evidence only. Atlas tooling is merged (`07e1ab2e`) and ready for post-parity validation data. | Produce compact plots and manifest for Switzerland 72h and Canary d02 72h. TOST may resume only as optional secondary evidence and must not delay the tag. |
...
4. **TOST is no longer the v0.14 gate.** Case 3 completed and the TOST marathon
   was stopped before Case 4. It may resume later as secondary station sanity
   evidence, but the release closes on Switzerland 72h + Canary L2 d02 72h
   field-parity/stability and Grid-Delta Atlas.
