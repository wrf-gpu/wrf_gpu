...
Status (2026-06-10 12:12 WEST): **v0.14 is the active closure target. Grid-cell
...
|---|---|---|
| Step-1/grid-cell parity | Active. MYNN cold-start QKE, MYNN first-call surface semantics, WRF `LANDUSE.TBL`-backed `TSK/ZNT/MAVAIL`, WRF `phy_prep` thermodynamic inputs, local `SFCLAY1D_mynn` output algebra, MYNN dry-theta/phy_prep source coupling, WRF surface/land flux handoff localization, the JAX Step-1 NoahMP-disabled configuration gap, NoahMP land-tile energy forcing, the NoahMP/sfclay water-path moist-theta bug, and MYNN-EDMF `RTHBLTEN` are fixed or formally bounded at WRF internal hooks. Latest accepted proof `proofs/v014/mynn_rthblten_step1_closure.*` reconciles the previous same-input MYNN green proof with the operational strict red leaf: reassembled operational dry `T_TENDF` matches runtime to `4.55e-13`; MYNN drives the worst-cell max, but the strict field residual is RRTMG `RTHRATEN` dominated. Current strict Step-1 remains red at max_abs `53.52301833555157`, RMSE `2.5444971494115354`, p99 `16.631650419560028`; substituting WRF `RTHRATEN` collapses RMSE to `0.5433421347190945` and p99 to `0.8430227515735698`. | Active sprint: `.agent/sprints/2026-06-10-v014-gpt-rrtmg-rthraten-closure/`. Endpoint: close/bound the RRTMG clear-sky `RTHRATEN` field-dominant lane using `proofs/v014/rrtmg_step1_forcing_parity.*` as the starting localization. Do not respec the strict gate or start TOST/Switzerland-GPU until RRTMG is closed/bounded and an explicit manager/reviewer decision records the tolerance policy. |
