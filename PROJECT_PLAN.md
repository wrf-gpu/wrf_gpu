...
Status (2026-06-10 19:35 WEST): **v0.14 is the active closure target. The
...
| Switzerland/Gotthard | CPU72 truth is complete at `/mnt/data/wrf_gpu_validation/v014_switzerland_72h_cpu_20260610T122909Z/run_cpu`: 73 frames, `rc=0`, finite final frame, 24 dmpar ranks, resource CSVs captured. | Hold GPU launch until the nested-pipeline Noah-MP land fix is merged and short Canary land gate is green/bounded; Gotthard is land dominated, so the frozen-TSK bug would invalidate it. |
| Canary field parity | L2 d02 retained CPU-WRF truth is complete. Current GPU 72h run at `/mnt/data/wrf_gpu_validation/v014_canary_d02_72h_moistcqw_20260610T171818Z` is still running as a pre-fix stability baseline. h24/h36 compares show no renewed LBC/dycore runaway, but Fable proved a release-blocking missing-physics wiring bug: standalone nested pipeline never enables Noah-MP, so land `TSK` is frozen. Formal h36 failures are only `MUB`, `PB`, and marginal `QVAPOR`; surface/PBL/radiation residuals are consistent with the frozen-land lane. | Let the baseline run finish for trend evidence. In parallel, close `.agent/sprints/2026-06-10-v014-noahmp-nested-pipeline/`: wire Noah-MP into `nested_pipeline`, CPU-prove activation/carry seeding, then manager reruns exact-branch memory preflight and Canary h1-h4 land gate before full Canary/Switzerland 72h gates. |
