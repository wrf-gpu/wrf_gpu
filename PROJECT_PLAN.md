...
Status (2026-06-10 20:10 WEST): **v0.14 is the active closure target. The
...
TOST is secondary station sanity evidence only, not a tag gate.**
...
| Canary field parity | L2 d02 retained CPU-WRF truth is complete. The pre-fix GPU 72h run at `/mnt/data/wrf_gpu_validation/v014_canary_d02_72h_moistcqw_20260610T171818Z` is still running as a frozen-land stability baseline, not release-green evidence. h24/h36/h48 show no renewed LBC/dycore runaway, but Fable proved the missing-physics wiring bug: standalone nested pipeline never enabled Noah-MP, so land `TSK` was frozen. Source fix is now merged/pushed: `c2310c5b` activates/seeds Noah-MP per nested domain when `sf_surface_physics=4`; `c6800bfa` adds the offline h1-h4 land-gate scorer. CPU proof: `proofs/v014/noahmp_nested_pipeline_activation.*`, verdict `NOAHMP_NESTED_ACTIVATION_CPU_PROVEN`; 32 focused/regression tests pass. | Let the pre-fix baseline finish for trend evidence. A lock-based watcher is armed to run the post-fix exact-branch memory preflight and Canary h1-h4 land gate as soon as the current GPU lock releases. If h1-h4 is green/bounded, launch full Canary 72h from the fixed candidate, then Switzerland 72h. |
