...
Status (2026-06-10 07:23 WEST): **v0.14 is the active closure target. Grid-cell
...
|---|---|---|
| Step-1/grid-cell parity | Active. MYNN cold-start QKE, MYNN first-call surface semantics, WRF `LANDUSE.TBL`-backed `TSK/ZNT/MAVAIL`, WRF `phy_prep` thermodynamic inputs, local `SFCLAY1D_mynn` output algebra, MYNN dry-theta/phy_prep source coupling, WRF surface/land flux handoff localization, and the JAX Step-1 NoahMP-disabled configuration gap are fixed/bounded at WRF internal hooks. The latest proof (`proofs/v014/noahmp_step1_closure.*`) reports `use_noahmp=True`, `sf_surface_physics=4`, and WRF-derived NoahMP land/static/radiation state in the Step-1 path. Strict after-conv `T_TENDF` remains red against pinned one-run WRF truth at max_abs `1489.5135568470864`, RMSE `13.2001844004901`. The active blocker is now the NoahMP land-tile energy/HFX chain at strict worst cells; secondary RRTMG GLW/RTHRATEN parity remains open. | Dispatch the NoahMP land-tile ENERGY sprint as one whole hard task, preferably to Fable/Mythos after `/compact`: emit per-column WRF `noahmplsm` energy in/out hooks at the worst cell, compare to `physics.noahmp`, fix the diverging chain, then rerun `proofs/v014/noahmp_step1_closure.py`. A separate GPT CPU-only RRTMG GLW/RTHRATEN parity sprint can run in parallel if file ownership stays disjoint. No TOST/Switzerland/FP32 until strict Step-1 is fixed or explicitly bounded for release. |
