...
Status (2026-06-10 21:40 WEST): **v0.14 is the active closure target. The
...
TOST is secondary station sanity evidence only, not a tag gate. Current blocker:
post-NoahMP exact-branch preflight fits memory but fails functionally on a
bounded d01 Noah-MP LU16 nonfinite land-surface edge; Fable high is debugging in
`.claude/worktrees/fable-noahmp-d01-lu16`.**
...
| Memory/FP32 | Memory lane accepted and merged: MYNN BouLac leading-column tiling (`26815feb`) cuts the measured whole-batch MYNN temp from 14.7 GiB to 3.2 GiB on the 641x321x50 target; shared RK-stage transport velocities landed as exact hygiene. The latest post-NoahMP exact-branch preflight is memory-green but functionally red: `/mnt/data/wrf_gpu_validation/v014_noahmp_l2_preflight_20260610T192315Z`, `rc=1`, peak total VRAM `10042 MiB`, compute-app `9161 MiB`, no OOM markers; the failure is d01 LU16 nonfinites, not memory. FP32 R0 default-inert acoustic precision-mode contract landed (`bc847db2`), but R1/R2 mixed precision remains blocked until the fp64 validation frontier is closed. | Do not spend Fable/Mythos on memory now. After the d01 LU16 blocker is fixed/bounded, rerun exact-branch memory preflight on the final candidate. FP32 R1 is v0.15 unless field-gate failure forces a v0.14 revisit. |
...
| Switzerland/Gotthard | CPU72 truth is complete at `/mnt/data/wrf_gpu_validation/v014_switzerland_72h_cpu_20260610T122909Z/run_cpu`: 73 frames, `rc=0`, finite final frame, 24 dmpar ranks, resource CSVs captured. | Hold GPU launch until the post-merge Canary h1-h4 Noah-MP land gate is green/bounded; Gotthard is land dominated, so any remaining frozen-TSK path would invalidate it. |
| Canary field parity | L2 d02 retained CPU-WRF truth is complete. The pre-fix GPU 72h run at `/mnt/data/wrf_gpu_validation/v014_canary_d02_72h_moistcqw_20260610T171818Z` completed as a frozen-land stability baseline, not release-green evidence. h24/h36/h48 show no renewed LBC/dycore runaway, but Fable proved the missing-physics wiring bug: standalone nested pipeline never enabled Noah-MP, so land `TSK` was frozen. Source fix is merged/pushed: `c2310c5b` activates/seeds Noah-MP per nested domain when `sf_surface_physics=4`; `c6800bfa` adds the offline h1-h4 land-gate scorer. The post-fix exact-branch preflight at `/mnt/data/wrf_gpu_validation/v014_noahmp_l2_preflight_20260610T192315Z` failed `rc=1`: memory fit is green, d02 finite, but d01 has 51 nonfinite LU16 land cells in `T2/UST/HFX/LH/TSK/TH2/LWUPB/LWUPT/OLR`. | Fable high is active in tmux `0:3` on branch `worker/fable/v014-noahmp-d01-lu16` with contract `.agent/sprints/2026-06-10-v014-noahmp-d01-lu16-nonfinite/sprint-contract.md`. After fix/bound: rerun exact-branch preflight, then h1-h4 land gate. If green/bounded, launch full Canary 72h from the fixed candidate, then Switzerland 72h. |
